...
use mcandle_metal_kernels::{BufferOffset, CallConvTranspose2dCfg, Kernels};
...
pub enum MetalError {
    ...
    Message(String),
    #[error(transparent)]
    KernelError(#[from] mcandle_metal_kernels::MetalKernelError),
    #[error("{0:?}")]
    LockError(LockError),
...
impl BackendStorage for MetalStorage {
    ...
    fn affine(&self, layout: &Layout, mul: f64, add: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            mcandle_metal_kernels::call_affine(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
            ...
            mcandle_metal_kernels::call_affine_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn powf(&self, layout: &Layout, pow: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            mcandle_metal_kernels::call_powf(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
            ...
            mcandle_metal_kernels::call_powf_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn elu(&self, layout: &Layout, alpha: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            mcandle_metal_kernels::call_elu(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
            ...
            mcandle_metal_kernels::call_elu_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn reduce_op(&self, op: ReduceOp, layout: &Layout, sum_dims: &[usize]) -> Result<Self> {
        ...
        let command_buffer = self.device.command_buffer()?;
        let src = buffer_o(&self.buffer, layout, self.dtype);
        mcandle_metal_kernels::call_reduce_strided(
            &device.device,
            &command_buffer,
            &device.kernels,
            name,
    ...
    fn to_dtype(&self, layout: &Layout, dtype: DType) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            mcandle_metal_kernels::call_cast_contiguous(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
            ...
            mcandle_metal_kernels::call_cast_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
    ...
    fn where_cond(
        ...
        mcandle_metal_kernels::call_where_cond_strided(
            &device.device,
            &command_buffer,
            &device.kernels,
            name,
    ...
    fn conv1d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        mcandle_metal_kernels::call_im2col1d_strided(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose1d(
        ...
        let buffer = if USE_COL2IM_CONV1D_TR && can_use_col2im {
            ...
            let command_buffer = self.device.command_buffer()?;
            mcandle_metal_kernels::call_col2im1d(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                name,
            ...
            mcandle_metal_kernels::call_conv_transpose1d(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                name,
    ...
    fn conv2d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        mcandle_metal_kernels::call_im2col_strided(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose2d(
        ...
        mcandle_metal_kernels::call_conv_transpose2d(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn avg_pool2d(
        ...
        mcandle_metal_kernels::call_pool2d(
            &self.device.device,
            &command_buffers,
            &self.device.kernels,
            name,
    ...
    fn upsample_nearest2d(&self, inp_l: &Layout, out_w: usize, out_h: usize) -> Result<Self> {
        ...
        mcandle_metal_kernels::call_upsample_nearest_2d(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn gather(&self, src_l: &Layout, ids: &Self, ids_l: &Layout, dim: usize) -> Result<Self> {
        ...
        mcandle_metal_kernels::call_gather(
            &device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn scatter_add(
        ...
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        mcandle_metal_kernels::call_scatter_add(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn index_select(&self, ids: &Self, src_l: &Layout, ids_l: &Layout, dim: usize) -> Result<Self> {
        ...
        let src = buffer_o(&self.buffer, src_l, dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        mcandle_metal_kernels::call_index_select(
            &device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn index_add(
        ...
        let src = buffer_o(&src.buffer, src_l, src.dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        mcandle_metal_kernels::call_index_add(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn matmul_with_alpha_beta(
        ...
        mcandle_metal_kernels::call_gemm(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn matmul_with_alpha(
        ...
        if self.dtype == DType::BF16 {
            ...
            mcandle_metal_kernels::call_mlx_gemm(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                mcandle_metal_kernels::GemmDType::BF16,
                (b, m, n, k),
                lhs_l.stride(),
                lhs_l.start_offset() * self.dtype.size_in_bytes(),
                &self.buffer,
            ...
            let dtype = match self.dtype {
                DType::F32 => mcandle_metal_kernels::GemmDType::F32,
                DType::F16 => mcandle_metal_kernels::GemmDType::F16,
                DType::BF16 => mcandle_metal_kernels::GemmDType::BF16,
                dtype => {
    ...
    fn copy2d(
        ...
        if src_s == d2 && dst_s == d2 {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => mcandle_metal_kernels::copy2d::FLOAT,
                DType::F16 => mcandle_metal_kernels::copy2d::HALF,
                DType::BF16 => mcandle_metal_kernels::copy2d::BFLOAT,
                DType::I16 => mcandle_metal_kernels::copy2d::I16,
                DType::I32 => mcandle_metal_kernels::copy2d::I32,
                DType::I64 => mcandle_metal_kernels::copy2d::I64,
                DType::U32 => mcandle_metal_kernels::copy2d::U32,
                DType::U8 => mcandle_metal_kernels::copy2d::U8,
                dtype => crate::bail!("Metal copy2d {dtype:?} not implemented"),
            };
            mcandle_metal_kernels::call_copy2d(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                kernel_name,
    ...
    fn copy_strided_src(&self, dst: &mut Self, dst_offset: usize, src_l: &Layout) -> Result<()> {
        ...
        if src_l.is_contiguous() && self.dtype == dst.dtype() {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => mcandle_metal_kernels::unary::strided::copy::FLOAT,
                DType::F16 => mcandle_metal_kernels::unary::strided::copy::HALF,
                DType::BF16 => mcandle_metal_kernels::unary::strided::copy::BFLOAT,
                DType::I16 => mcandle_metal_kernels::unary::strided::copy::I16,
                DType::I32 => mcandle_metal_kernels::unary::strided::copy::I32,
                DType::I64 => mcandle_metal_kernels::unary::strided::copy::I64,
                DType::U32 => mcandle_metal_kernels::unary::strided::copy::U32,
                DType::U8 => mcandle_metal_kernels::unary::strided::copy::U8,
...
impl MetalStorage {
    ...
    pub fn binary(
        ...
        let (buffer, dtype) = if lhs_l.is_contiguous() && rhs_l.is_contiguous() && &op[..1] != "b" {
            use mcandle_metal_kernels::binary::contiguous;
            ...
            mcandle_metal_kernels::call_binary_contiguous(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
            ...
            use mcandle_metal_kernels::binary::strided;
            ...
            let buffer = device.new_buffer(el_count, dtype, op)?;
            mcandle_metal_kernels::call_binary_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
...
impl BackendDevice for MetalDevice {
    ...
    fn ones_impl(&self, shape: &Shape, dtype: DType) -> Result<MetalStorage> {
        ...
        mcandle_metal_kernels::call_const_fill(
            &self.device,
            &command_buffer,
            &self.kernels,
            name,
    ...
    fn rand_uniform(
        ...
        let command_buffer = self.command_buffer()?;
        mcandle_metal_kernels::call_random_uniform(
            &self.device,
            &command_buffer,
            &self.kernels,
            name,
    ...
    fn rand_normal(
        ...
        let command_buffer = self.command_buffer()?;
        mcandle_metal_kernels::call_random_normal(
            &self.device,
            &command_buffer,
            &self.kernels,
            name,

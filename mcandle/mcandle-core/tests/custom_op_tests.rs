...
use mcandle_core::cpu_backend;
use mcandle_core::test_utils::to_vec1_round;
use mcandle_core::{CpuStorage, CustomOp1, DType, Device, Error, Layout, Result, Shape, Tensor};
...
impl CustomOp1 for Elu {
    ...
    fn cpu_fwd(&self, s: &CpuStorage, l: &Layout) -> Result<(CpuStorage, Shape)> {
        let storage = mcandle_core::map_dtype!(
            "elu",
            s,
...
fn custom_op1_with_backward() -> Result<()> {
    let cpu = &Device::Cpu;
    let t = mcandle_core::Var::new(&[-2f32, 0f32, 2f32], cpu)?;
...
impl mcandle_core::InplaceOp1 for Elu {
    fn name(&self) -> &'static str {
        "elu"
    }
    ...
    fn cpu_fwd(&self, s: &mut CpuStorage, _l: &Layout) -> Result<()> {
        ...
        match s {
            ...
            _ => mcandle_core::bail!("unsupported dtype for inplace elu"),
        }
        Ok(())
    }
}
...
fn ug_op() -> Result<()> {
    ...
    let device = if mcandle_core::utils::cuda_is_available() {
        Device::new_cuda(0)?
    } else if mcandle_core::utils::metal_is_available() {
        Device::new_metal(0)?
    } else {
        mcandle_core::bail!("metal/cuda is mandatory for this test")
    };
    let op = mcandle_core::UgIOp1::new("test", kernel, &device)?;

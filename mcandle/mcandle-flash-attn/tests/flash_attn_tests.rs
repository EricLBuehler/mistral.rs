use anyhow::Result;
use mcandle_core::{DType, Device, IndexOp, Tensor, D};
...
fn fa_acausal(
    ...
    let att = mcandle_nn::ops::softmax(&att, D::Minus1)?;
...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        mcandle_flash_attn::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        mcandle_flash_attn::flash_attn_varlen(
...
fn flash_attn_acausal_softcap() -> Result<()> {
    ...
    let ys2 = {
        ...
        let k = k.transpose(1, 2)?;
        let v = v.transpose(1, 2)?;
        mcandle_flash_attn::flash_attn_softcap(&q, &k, &v, 0.5, Some(30.), false)?.transpose(1, 2)?
    };
    let ys2 = ys2.i(0)?.to_dtype(DType::F32)?;
    let diff = ys1.sub(&ys2)?.abs()?.flatten_all()?.max(0)?;
...
fn flash_attn_varlen_softcap() -> Result<()> {
    ...
    let ys = {
        ...
        let k = k.transpose(0, 1)?;
        let v = v.transpose(0, 1)?;
        mcandle_flash_attn::flash_attn_varlen_softcap(
            &q,
            &k,
            &v,
            &seqlens_q,

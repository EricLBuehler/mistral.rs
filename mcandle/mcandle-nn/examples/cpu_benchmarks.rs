...
use mcandle_core::quantized::GgmlType;
use mcandle_core::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl mcandle_core::CustomOp1 for Im2Col {
...
impl Benchmark for QMatMul {
    type PreProcessData = (mcandle_core::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![mcandle_core::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = mcandle_core::quantized::QTensor::new(
            mcandle_core::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = mcandle_core::quantized::QMatMul::from_qtensor(mm)?;
...
impl Benchmark for Softmax {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        mcandle_nn::ops::softmax(d, D::Minus1)
    }
...
impl Benchmark for SoftmaxLastDim {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        mcandle_nn::ops::softmax_last_dim(d)
    }

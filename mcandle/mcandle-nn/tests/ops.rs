...
use mcandle_core::{test_device, test_utils::to_vec3_round, Device, Result, Tensor};

fn softmax(device: &Device) -> Result<()> {
    let data = &[[[3f32, 1., 4.], [1., 5., 9.]], [[2., 1., 7.], [8., 2., 8.]]];
    let tensor = Tensor::new(data, device)?;
    let t0 = mcandle_nn::ops::softmax(&tensor.log()?, 0)?;
    let t1 = mcandle_nn::ops::softmax(&tensor.log()?, 1)?;
    let t2 = mcandle_nn::ops::softmax(&tensor.log()?, 2)?;
    assert_eq!(
        to_vec3_round(&t0, 4)?,
        &[
            // 3/5, 1/2, 4/11
    ...
    let t2 = mcandle_nn::ops::softmax_last_dim(&tensor.log()?)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            // (3, 1, 4) / 8, (1, 5, 9) / 15
...
fn rms_norm(device: &Device) -> Result<()> {
    ...
    let t = mcandle_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t, 4)?,
        &[
    ...
    let t2 = mcandle_nn::ops::rms_norm_slow(&tensor, &alpha, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            [[1.019, 0.6794, 4.0762], [0.1674, 1.6744, 4.521]],
...
fn rms_norml(device: &Device) -> Result<()> {
    ...
    let alpha = Tensor::ones(head_dim, mcandle_core::DType::F32, device)?;
    let t = mcandle_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    let t2 = mcandle_nn::ops::rms_norm_slow(&tensor, &alpha, 1e-5)?;
    let diff = (t - t2)?
        .abs()?
...
fn layer_norm(device: &Device) -> Result<()> {
    ...
    let t = mcandle_nn::ops::layer_norm(&tensor, &alpha, &beta, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t, 4)?,
        &[
    ...
    let t2 = mcandle_nn::ops::layer_norm_slow(&tensor, &alpha, &beta, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            [[0.7673, -2.6726, 3.0071], [-0.7247, 0.0, 3.4742]],
...
fn layer_norml(device: &Device) -> Result<()> {
    ...
    let tensor = Tensor::new(src, device)?.reshape((b_size, seq_len, head_dim))?;
    let alpha = Tensor::ones(head_dim, mcandle_core::DType::F32, device)?;
    let beta = Tensor::zeros(head_dim, mcandle_core::DType::F32, device)?;
    let t = mcandle_nn::ops::layer_norm(&tensor, &alpha, &beta, 1e-5)?;
    let t2 = mcandle_nn::ops::layer_norm_slow(&tensor, &alpha, &beta, 1e-5)?;
    let diff = (t - t2)?
        .abs()?
...
fn softmax_numerical_stability() -> Result<()> {
    ...
    let softmax = mcandle_nn::ops::softmax(&xs, 0)?;
...
fn ropei(device: &Device) -> Result<()> {
    ...
    let rope1 = mcandle_nn::rotary_emb::rope_i(&src, &cos, &sin)?;
    let rope2 = mcandle_nn::rotary_emb::rope_i_slow(&src, &cos, &sin)?;
...
fn rope(device: &Device) -> Result<()> {
    ...
    let cos = Tensor::from_vec(cos, (seq_len, head_dim / 2), device)?;
    let sin = Tensor::from_vec(sin, (seq_len, head_dim / 2), device)?;
    let rope1 = mcandle_nn::rotary_emb::rope(&src, &cos, &sin)?;
    let rope2 = mcandle_nn::rotary_emb::rope_slow(&src, &cos, &sin)?;
    let sum_diff = (rope1 - rope2)?.abs()?.sum_all()?.to_vec0::<f32>()?;
    if device.is_cpu() {
        assert_eq!(sum_diff, 0.);
    } else {
...
fn rope_thd(device: &Device) -> Result<()> {
    ...
    let rope1 = {
        ...
        mcandle_nn::rotary_emb::rope_thd(&src, &cos, &sin)?.transpose(1, 2)?
    };
    let rope2 = mcandle_nn::rotary_emb::rope_slow(&src, &cos, &sin)?;
    let sum_diff = (rope1 - rope2)?.abs()?.sum_all()?.to_vec0::<f32>()?;
    if device.is_cpu() {
...
fn sigmoid(device: &Device) -> Result<()> {
    let data = &[[[3f32, 1., 4.], [1., 5., 9.]], [[2., 1., 7.], [8., 2., 8.]]];
    let tensor = Tensor::new(data, device)?;
    let s1 = mcandle_nn::ops::sigmoid(&tensor)?;

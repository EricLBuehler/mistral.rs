...
use mcandle_core::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use mcandle_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = mcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = mcandle_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = mcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = mcandle_nn::gru(2, 3, Default::default(), vb)?;

...
mod metal_sdpa_tests {
    #[test]
    fn sdpa_full() -> mcandle_core::Result<()> {
        ...
        let ground_truth = {
            ...
            let att = mcandle_nn::ops::softmax_last_dim(&att.to_dtype(DType::F32)?)?
                .to_dtype(q.dtype())?;
            att.matmul(&v.clone())?
        };

        let sdpa_output = mcandle_nn::ops::sdpa(&q, &k, &v, scale as f32, 1.)?;
    ...
    #[test]
    fn sdpa_vector() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};
    ...
    #[test]
    fn sdpa_full_softcapping() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};
        use std::ops::{Div, Mul};
        ...
        let ground_truth = {
            let att = (q.clone() * scale)?.matmul(&k.clone().t()?)?;
            let att = mcandle_nn::ops::softmax_last_dim(
        ...
        let sdpa_output = mcandle_nn::ops::sdpa(&q, &k, &v, scale as f32, SOFTCAP as f32)?;
    ...
    #[test]
    fn sdpa_vector_softcapping() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};
        use std::ops::{Div, Mul};
    ...
    #[test]
    fn sdpa_vector_cross() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};
    ...
    #[test]
    fn attn_softmax_mask() -> mcandle_core::Result<()> {
        ...
        let ground_truth = mcandle_nn::ops::softmax_last_dim(&tensor.broadcast_add(&truemask)?)?;

        let softmax_out = mcandle_nn::ops::attn_softmax_last_dim(&tensor, &truemask, 1.)?;
    ...
    #[test]
    fn attn_softmax_mask_scale() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};
        ...
        let ground_truth =
            mcandle_nn::ops::softmax_last_dim(&(tensor.broadcast_add(&truemask)? * scale as f64)?)?
                .to_dtype(DType::F32)?;

        let softmax_out = mcandle_nn::ops::attn_softmax_last_dim(&tensor, &truemask, scale)?
            .to_dtype(DType::F32)?;
    ...
    #[test]
    fn attn_softmax_mask_novec() -> mcandle_core::Result<()> {
        use mcandle_core::{Device, Tensor};
    ...
    #[test]
    fn attn_softmax_mask_scale_novec() -> mcandle_core::Result<()> {
        use mcandle_core::{DType, Device, Tensor};

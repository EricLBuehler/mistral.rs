...
use crate::VarMap;
use mcandle_core::{safetensors::Load, DType, Device, Error, Result, Shape, Tensor};
...
impl SimpleBackend for HashMap<String, Tensor> {
    fn get(
        ...
        let tensor = self
            ...
            .clone();
        if tensor.shape() != &s {
            Err(mcandle_core::Error::UnexpectedShape {
...
impl SimpleBackend for mcandle_core::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for mcandle_core::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for mcandle_core::safetensors::MmapedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for mcandle_core::safetensors::BufferedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> SimpleBackend for mcandle_core::safetensors::SliceSafetensors<'a> {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub unsafe fn from_mmaped_safetensors<P: AsRef<std::path::Path>>(
        ...
        let tensors = mcandle_core::safetensors::MmapedSafetensors::multi(paths)?;
    ...
    pub fn from_buffered_safetensors(data: Vec<u8>, dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = mcandle_core::safetensors::BufferedSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_slice_safetensors(data: &'a [u8], dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = mcandle_core::safetensors::SliceSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = mcandle_core::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = mcandle_core::pickle::PthTensors::new(p, None)?;
    ...
    ///
    /// ```rust
    /// use mcandle_core::{Tensor, DType, Device};
    ///
    ...
    /// .into_iter()
    /// .collect();
    /// let vb = mcandle_nn::VarBuilder::from_tensors(tensors, DType::F32, &Device::Cpu);
...
pub struct ShardedSafeTensors(mcandle_core::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            mcandle_core::bail!("Get sharded on dimensions != 0 or 1")
        };

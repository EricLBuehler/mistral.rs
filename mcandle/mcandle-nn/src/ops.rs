...
use mcandle_core::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use mcandle_core::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = mcandle_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
///     ]);
/// # Ok::<(), mcandle_core::Error>(())
/// ```
pub fn softmax<D: mcandle_core::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: mcandle_core::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl mcandle_core::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use mcandle_core::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `mcandle_core::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(mcandle_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(mcandle_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(mcandle_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(mcandle_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(mcandle_core::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        ...
        use mcandle_core::cuda_backend::SlicePtrOrNull;
        use mcandle_core::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use mcandle_core::{CudaDevice, WithDType};
        ...
        let dst = mcandle_core::CudaStorage {
            slice,
            device: dev.clone(),
        };
    ...
    fn metal_fwd(
        &self,
        storage: &mcandle_core::MetalStorage,
        layout: &Layout,
    ) -> Result<(mcandle_core::MetalStorage, Shape)> {
        use mcandle_core::backend::BackendStorage;
        use mcandle_core::MetalError;
        ...
        let src = mcandle_metal_kernels::BufferOffset {
        ...
        match (el_count % 2, dtype, layout.is_contiguous()) {
            (0, DType::BF16 | DType::F16, true) => {
                use mcandle_metal_kernels::unary::contiguous_tiled;
                ...
                mcandle_metal_kernels::call_unary_contiguous_tiled(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, true) => {
                use mcandle_metal_kernels::unary::contiguous;
                let kernel_name = match dtype {
                ...
                mcandle_metal_kernels::call_unary_contiguous(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, false) => {
                use mcandle_metal_kernels::unary::strided;
                let kernel_name = match dtype {
                    DType::F16 => strided::sigmoid::HALF,
                    DType::F32 => strided::sigmoid::FLOAT,
                    DType::BF16 => strided::sigmoid::BFLOAT,
                    dtype => {
                        mcandle_core::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                    }
                };
                let dst = mcandle_metal_kernels::BufferOffset::zero_offset(&buffer);
                mcandle_metal_kernels::call_unary_strided(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
        ...
        let new_storage = mcandle_core::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        mcandle_core::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl mcandle_core::ModuleT for Dropout {
...
impl mcandle_core::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: mcandle_core::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => mcandle_core::bail!("input has to be contiguous"),
            ...
            let storage = mcandle_core::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => mcandle_core::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = mcandle_core::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => mcandle_core::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            mcandle_core::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        mcandle_metal_kernels::call_last_softmax(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        let newstorage =
            mcandle_core::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl mcandle_core::CustomOp2 for AttnSoftmaxLastDim {
    fn name(&self) -> &'static str {
    ...
    fn cpu_fwd(
        ...
        mcandle_core::bail!("cpu attn-softmax-last-dim is not implemented");
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        a_s: &mcandle_core::MetalStorage,
        a_l: &Layout,
        mask_s: &mcandle_core::MetalStorage,
        mask_l: &Layout,
    ) -> Result<(mcandle_core::MetalStorage, Shape)> {
        use mcandle_core::backend::BackendStorage;
        ...
        let ty = match a_s.dtype() {
            DType::F32 => mcandle_metal_kernels::SdpaDType::F32,
            DType::F16 => mcandle_metal_kernels::SdpaDType::F16,
            DType::BF16 => mcandle_metal_kernels::SdpaDType::BF16,
            dtype => mcandle_core::bail!("attn-softmax-last-dim is not implemented for {dtype:?}"),
        };

        if !a_l.is_contiguous() {
            mcandle_core::bail!("Non contiguous xs for attn-softmax-last-dim is not implemented");
        }
        if !mask_l.is_contiguous() {
            mcandle_core::bail!("Non contiguous mask for attn-softmax-last-dim is not implemented");
        }

        if a_l.dims().len() != 4 {
            mcandle_core::bail!("attn-softmax-last-dim expects xs of rank 2");
        }
        if mask_l.dims().len() != 2 {
            mcandle_core::bail!("attn-softmax-last-dim expects mask of rank 2");
        }
        if mask_l.dim(D::Minus1)? != a_l.dim(D::Minus1)?
            || mask_l.dim(D::Minus2)? != a_l.dim(D::Minus2)?
        {
            mcandle_core::bail!(
                "attn-softmax-last-dim expects last 2 dims to match xs last 2 dims"
            );
        }

        let elem_count = a_l.shape().elem_count();
        let output = device.new_buffer(elem_count, a_s.dtype(), "attn-softmax")?;
        mcandle_metal_kernels::call_last_attn_softmax(
            device.metal_device(),
            &command_buffer,
            kernels,
            a_s.buffer(),
        ...
        .map_err(mcandle_core::Error::wrap)?;
        let newstorage =
            mcandle_core::MetalStorage::new(output, device.clone(), elem_count, a_s.dtype());
...
/// Equivalent to:
/// ```ignore
/// mcandle_nn::ops::softmax_last_dim(&(xs.broadcast_add(&mask)? * scale as f64)?)?
/// ```
...
impl mcandle_core::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => mcandle_core::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => mcandle_core::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &mcandle_core::CudaStorage,
        l1: &Layout,
        s2: &mcandle_core::CudaStorage,
        l2: &Layout,
    ) -> Result<(mcandle_core::CudaStorage, Shape)> {
        use mcandle_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use mcandle_core::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use mcandle_core::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => mcandle_core::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            mcandle_core::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        mcandle_metal_kernels::call_rms_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        .map_err(mcandle_core::Error::wrap)?;
        let newstorage =
            mcandle_core::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
...
impl mcandle_core::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => mcandle_core::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &mcandle_core::CudaStorage,
        l2: &Layout,
        s3: &mcandle_core::CudaStorage,
        l3: &Layout,
    ) -> Result<(mcandle_core::CudaStorage, Shape)> {
        use mcandle_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use mcandle_core::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use mcandle_core::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                mcandle_core::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            mcandle_core::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        mcandle_metal_kernels::call_layer_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => mcandle_core::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
pub fn kvconcat(ltensor: &Tensor, rtensor: &Tensor, concat_dim: usize) -> Result<Tensor> {
    ...
    use mcandle_core::cuda_backend::KVConcat;
...
impl mcandle_core::CustomOp3 for Sdpa {
    fn name(&self) -> &'static str {
        "metal-sdpa"
    }
    ...
    fn cpu_fwd(
        ...
        mcandle_core::bail!("SDPA has no cpu impl")
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        q: &mcandle_core::MetalStorage,
        q_l: &Layout,
        k: &mcandle_core::MetalStorage,
        k_l: &Layout,
        v: &mcandle_core::MetalStorage,
        v_l: &Layout,
    ) -> Result<(mcandle_core::MetalStorage, Shape)> {
        use mcandle_core::backend::BackendStorage;
        use mcandle_metal_kernels::SdpaDType;
        ...
        if q_l.dim(D::Minus1)? != k_l.dim(D::Minus1)? {
            mcandle_core::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dim(D::Minus(3))? != k_l.dim(D::Minus(3))? {
            mcandle_core::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dim(D::Minus(3))? % k_l.dim(D::Minus(3))? != 0 {
            mcandle_core::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                mcandle_core::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        let itype = match q.dtype() {
            ...
            other => mcandle_core::bail!("unsupported sdpa type {other:?}"),
        };

        let command_buffer = q.device().command_buffer()?;
        if supports_sdpa_vector {
            command_buffer.set_label("vector_attention");
            mcandle_metal_kernels::call_sdpa_vector(
                q.device().device(),
                &command_buffer,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            mcandle_metal_kernels::call_sdpa_full(
                q.device().device(),
                &command_buffer,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            mcandle_core::bail!("must be vector or full sdpa kernel");
        }

        let newstorage =
            mcandle_core::MetalStorage::new(output, device.clone(), elem_count, q.dtype());

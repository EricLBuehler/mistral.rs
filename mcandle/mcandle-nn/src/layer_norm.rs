...
//!
//! ```rust
//! use mcandle_core::{Tensor, Device::Cpu, test_utils::to_vec3_round};
//! use mcandle_nn::{LayerNorm, Module};
//! # fn main() -> mcandle_core::Result<()> {
//!
...
use mcandle_core::{DType, Module, Result, Tensor, D};
        ...
        match (
            ...
            (Storage::Cuda(x_storage), Storage::Cuda(weight_storage)) => {
                match (x_storage.dtype(), weight_storage.dtype()) {
                    ...
                    _ => mcandle_core::bail!("DType mismatch in fused rmsnorm."),
                }
            }
            _ => unreachable!(),
        }

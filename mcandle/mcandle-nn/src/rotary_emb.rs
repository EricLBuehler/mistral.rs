//! Rotary Embeddings
//!
use mcandle_core::{CpuStorage, Layout, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
impl mcandle_core::CustomOp3 for RotaryEmbI {
    ...
    fn cpu_fwd(
        ...
        fn inner<T: mcandle_core::WithDType + num_traits::Float>(
            src: &[T],
            l_src: &Layout,
            cos: &[T],
            l_cos: &Layout,
            ...
            let src = match l_src.contiguous_offsets() {
                None => mcandle_core::bail!("input src has to be contiguous"),
                Some((o1, o2)) => &src[o1..o2],
            };
            let cos = match l_cos.contiguous_offsets() {
                None => mcandle_core::bail!("input cos has to be contiguous"),
                Some((o1, o2)) => &cos[o1..o2],
            };
            let sin = match l_sin.contiguous_offsets() {
                None => mcandle_core::bail!("input sin has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        use mcandle_core::cuda_backend::{kernel_name, kernels, WrapErr};
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            let src = match l_src.contiguous_offsets() {
                None => mcandle_core::bail!("src input has to be contiguous"),
                Some((o1, o2)) => src.slice(o1..o2),
            };
            let cos = match l_cos.contiguous_offsets() {
                None => mcandle_core::bail!("cos input has to be contiguous"),
                Some((o1, o2)) => cos.slice(o1..o2),
            };
            let sin = match l_sin.contiguous_offsets() {
                None => mcandle_core::bail!("sin input has to be contiguous"),
        ...
        use mcandle_core::backend::BackendStorage;
        use mcandle_core::cuda_backend::CudaStorageSlice::{BF16, F16, F32, F64};
        let dev = s1.device();
    ...
    fn metal_fwd(
        &self,
        src: &mcandle_core::MetalStorage,
        l_src: &Layout,
        cos: &mcandle_core::MetalStorage,
        l_cos: &Layout,
        sin: &mcandle_core::MetalStorage,
        l_sin: &Layout,
        ...
        let name = match src.dtype() {
            mcandle_core::DType::F32 => "rope_i_f32",
            mcandle_core::DType::F16 => "rope_i_f16",
            mcandle_core::DType::BF16 => "rope_i_bf16",
            dtype => mcandle_core::bail!("rope-i is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-i")?;
        mcandle_metal_kernels::call_rope_i(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        let out = mcandle_core::MetalStorage::new(output, device.clone(), el, src.dtype());
...
pub fn rope_i(xs: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    if !xs.is_contiguous() {
        mcandle_core::bail!("xs has to be contiguous in rope")
    }
    if !cos.is_contiguous() {
        mcandle_core::bail!("cos has to be contiguous in rope")
    }
    if !sin.is_contiguous() {
        mcandle_core::bail!("sin has to be contiguous in rope")
    }
...
impl mcandle_core::CustomOp3 for RotaryEmb {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            mcandle_core::DType::F32 => "rope_f32",
            mcandle_core::DType::F16 => "rope_f16",
            mcandle_core::DType::BF16 => "rope_bf16",
            dtype => mcandle_core::bail!("rope is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-i")?;
        mcandle_metal_kernels::call_rope(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
...
impl mcandle_core::CustomOp3 for RotaryEmbThd {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            mcandle_core::DType::F32 => "rope_thd_f32",
            mcandle_core::DType::F16 => "rope_thd_f16",
            mcandle_core::DType::BF16 => "rope_thd_bf16",
            dtype => mcandle_core::bail!("rope_thd is not implemented for {dtype:?}"),
        };
        let (b, t, h, d) = l_src.shape().dims4()?;
        let el = b * h * t * d;
        let output = device.new_buffer(el, src.dtype(), "rope-thd")?;
        mcandle_metal_kernels::call_rope_thd(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,

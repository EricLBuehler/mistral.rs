...
pub fn build_execute_python_tool(
    ...
    let video_desc = if supports_video {
        "- **Video/Animation/GIF**: Two methods are supported for producing animations:\n  **Method 1 (recommended):** Build frames manually with `_render_frame`:\n  ```python\n  _video_frames = []\n  for i in range(N):\n      fig, ax = plt.subplots()\n      # ... draw frame ...\n      _video_frames.append(_render_frame(fig))\n      plt.close(fig)\n  ```\n  **Method 2:** Use `matplotlib.animation.FuncAnimation` with `ani.save()` — frames are automatically captured as video.\n  Both methods produce video frames that are sent back to you. You can see and describe them. One video per execution."
    } else if supports_vision {
        "- **Video/Animation/GIF**: Two methods are supported for producing animations:\n  **Method 1 (recommended):** Build frames manually with `_render_frame`:\n  ```python\n  _video_frames = []\n  for i in range(N):\n      fig, ax = plt.subplots()\n      # ... draw frame ...\n      _video_frames.append(_render_frame(fig))\n      plt.close(fig)\n  ```\n  **Method 2:** Use `matplotlib.animation.FuncAnimation` with `ani.save()` — frames are automatically captured.\n  Frames are sent back as individual images. One video per execution."
    } else {
        "- **Video/Animation/GIF**: Two methods are supported for producing animations:\n  **Method 1 (recommended):** Build frames manually with `_render_frame`:\n  ```python\n  _video_frames = []\n  for i in range(N):\n      fig, ax = plt.subplots()\n      # ... draw frame ...\n      _video_frames.append(_render_frame(fig))\n      plt.close(fig)\n  ```\n  **Method 2:** Use `matplotlib.animation.FuncAnimation` with `ani.save()` — frames are automatically captured.\n  Frames are saved to the working directory. One video per execution."
    };

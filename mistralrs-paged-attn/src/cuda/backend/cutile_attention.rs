...
use super::{slice_ptr_mut_on_stream, slice_ptr_on_stream};
...
static WARMED_ATTENTION_LOCATIONS: OnceLock<Mutex<HashSet<DeviceLocation>>> = OnceLock::new();
static REGISTERED_ATTENTION_Q_GROUPS: OnceLock<Mutex<HashSet<usize>>> = OnceLock::new();

fn execution_context(dev: &CudaDevice) -> ExecutionContext {
    let stream = dev.cuda_stream();
    let ctx = stream.context();
    let cdev = unsafe {
        CutileDevice::borrow_raw(
            ctx.cu_ctx() as *mut c_void,
            ctx.cu_device() as c_int,
            ctx.ordinal(),
        )
    };
    let cstream = unsafe { CutileStream::borrow_raw(stream.cu_stream() as *mut c_void, &cdev) };
    ExecutionContext::new(cstream)
}

fn device_supported(dev: &CudaDevice) -> bool {
...
pub fn warmup_cutile_attention_kernels(device: &Device) -> Result<()> {
    ...
    {
        let mut warmed = WARMED_ATTENTION_LOCATIONS
            .get_or_init(|| Mutex::new(HashSet::new()))
            .lock()
            .unwrap();
    ...
    let result = warmup_cutile_attention_kernels_uncached(device);
    if result.is_err() {
        WARMED_ATTENTION_LOCATIONS
            .get_or_init(|| Mutex::new(HashSet::new()))
            .lock()
            .unwrap()
            .remove(&location);
...
fn warmup_cutile_attention_kernels_uncached(device: &Device) -> Result<()> {
    let q_groups = registered_attention_q_groups();
    if q_groups.is_empty() {
        return Ok(());
    }
    for q_group in q_groups {
        warmup_attention_q_group(device, q_group)?;
    }
    device.synchronize()
}

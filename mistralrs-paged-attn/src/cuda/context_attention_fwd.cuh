...
inline void context_attention_fwd_launcher(
    ...
    int padded_max_total_kv =
        CTX_DIVIDE_ROUND_UP(max_total_kv_len, BLOCK_SIZE) * BLOCK_SIZE;
    int logits_size = padded_max_total_kv * sizeof(float);
...
// ===== V2: Partitioned context attention (mirrors paged_attention_v2) =====

namespace vllm {

/// Partitioned context attention kernel.
///
/// Grid: (num_heads, total_new_tokens, max_num_partitions)
/// Each thread block handles one partition of the KV range for one query token.
///
/// The KV range is split into PARTITION_SIZE-token partitions. Each partition
/// computes local QK dot products, local softmax, and partial V accumulation.
/// A separate reduce kernel merges results across partitions.
template <typename scalar_t, typename cache_t, vllm::Fp8KVCacheDataType kv_dt,
          int HEAD_SIZE, int BLOCK_SIZE, int NUM_THREADS, int PARTITION_SIZE>
__global__ void context_attention_fwd_v2_kernel(
    float *__restrict__ exp_sums,       // [total_new_tokens, num_heads, max_num_partitions]
    float *__restrict__ max_logits,     // [total_new_tokens, num_heads, max_num_partitions]
    scalar_t *__restrict__ tmp_out,     // [total_new_tokens, num_heads, max_num_partitions, head_size]
    const scalar_t *__restrict__ q,     // [total_new_tokens, num_heads, head_size]
    const scalar_t *__restrict__ k,     // [total_new_tokens, num_kv_heads, head_size]
    const scalar_t *__restrict__ v,     // [total_new_tokens, num_kv_heads, head_size]
    const cache_t *__restrict__ k_cache,   // [num_blocks, num_kv_heads, head_size/x, block_size, x]
    ...
    const int *__restrict__ query_start_locs,
    ...
    const int head_idx = blockIdx.x;
    const int token_flat_idx = blockIdx.y;
    const int partition_idx = blockIdx.z;
    const int num_heads = gridDim.x;

    const int batch_idx = seq_ids[token_flat_idx];
    const int q_start = query_start_locs[batch_idx];
    const int q_local_idx = token_flat_idx - q_start;
    const int ctx_len = context_lens[batch_idx];
    const int q_len = query_lens[batch_idx];

    if (q_local_idx >= q_len) return;

    const int num_new_kv = q_local_idx + 1;
    const int total_kv_len = ctx_len + num_new_kv;

    // Partition token range
    const int part_start = partition_idx * PARTITION_SIZE;
    if (part_start >= total_kv_len) return;
    const int part_end = CTX_MIN(part_start + PARTITION_SIZE, total_kv_len);
    const int num_part_tokens = part_end - part_start;
...
#pragma unroll
    ...
    // Zero out logits
    for (int i = thread_idx; i < PARTITION_SIZE; i += NUM_THREADS) {
        logits[i] = 0.f;
    }
    __syncthreads();
    ...
    // ===== PHASE 1: Context tokens from paged KV cache =====
    // Process cache blocks that overlap with [part_start, min(part_end, ctx_len))
    if (part_start < ctx_len) {
        const int cache_start_block = part_start / BLOCK_SIZE;
        const int cache_end_token = CTX_MIN(part_end, ctx_len);
        const int cache_end_block = CTX_DIVIDE_ROUND_UP(cache_end_token, BLOCK_SIZE);

        for (int block_idx = cache_start_block + warp_idx;
             block_idx < cache_end_block; block_idx += NUM_WARPS) {
            const int64_t physical_block_number =
                static_cast<int64_t>(block_table[block_idx]);
...
#pragma unroll
                ...
                if (thread_group_offset == 0) {
                    const bool mask = token_idx >= ctx_len || token_idx < part_start;
                    bool sw_mask = false;
                    if (sliding_window > 0) {
                        const int q_global_pos = ctx_len + q_local_idx;
                        sw_mask = (q_global_pos - token_idx) >= sliding_window;
                    ...
                    const bool final_mask = mask || sw_mask;
                    logits[token_idx - part_start] = final_mask ? 0.f : qk;
                    qk_max = final_mask ? qk_max : fmaxf(qk_max, qk);
                }
            }
        }
    ...
    // Sync between Phase 1 and Phase 2 to prevent races at the
    // cache/input boundary (last cache block may overlap with first new block).
    __syncthreads();

    // ===== PHASE 2: New tokens from input K/V (causal) =====
    const int new_start_global = CTX_MAX(part_start, ctx_len);
    const int new_end_global = part_end;
    if (new_end_global > new_start_global) {
        const int new_start = new_start_global - ctx_len;
        const int new_end = new_end_global - ctx_len;
        const int new_start_block = new_start / BLOCK_SIZE;
        const int new_end_block = CTX_DIVIDE_ROUND_UP(new_end, BLOCK_SIZE);

        for (int block_idx = new_start_block + warp_idx;
             block_idx < new_end_block; block_idx += NUM_WARPS) {
            for (int i = 0; i < NUM_TOKENS_PER_THREAD_GROUP; i++) {
                const int local_offset =
                    (thread_group_idx + i * CTX_WARP_SIZE) % BLOCK_SIZE;
                const int new_token_idx = block_idx * BLOCK_SIZE + local_offset;
...
#pragma unroll
                ...
                if (thread_group_offset == 0) {
                    const bool mask = new_token_idx >= num_new_kv;
                    const int logit_idx = ctx_len + new_token_idx - part_start;
                    logits[logit_idx] = mask ? 0.f : qk;
                    qk_max = mask ? qk_max : fmaxf(qk_max, qk);
                }
            }
    ...
    // ===== Local softmax over this partition =====
#pragma unroll
    for (int mask = CTX_WARP_SIZE / 2; mask >= THREAD_GROUP_SIZE; mask /= 2) {
        qk_max = fmaxf(qk_max, VLLM_SHFL_XOR_SYNC(qk_max, mask));
    }
...
#pragma unroll
    ...
    // Sinks NOT included here — handled in reduce kernel.

    float exp_sum = 0.f;
    for (int i = thread_idx; i < num_part_tokens; i += NUM_THREADS) {
        float val = __expf(logits[i] - qk_max);
        logits[i] = val;
        exp_sum += val;
    }
...
#pragma unroll
    ...
    // Normalize logits (local softmax)
    const float inv_sum = __fdividef(1.f, exp_sum + 1e-6f);
    for (int i = thread_idx; i < num_part_tokens; i += NUM_THREADS) {
        logits[i] *= inv_sum;
    }
    ...
    // Store partition statistics to global memory
    if (thread_idx == 0) {
        const int stats_idx = token_flat_idx * num_heads * max_num_partitions
                            + head_idx * max_num_partitions + partition_idx;
        max_logits[stats_idx] = qk_max;
        exp_sums[stats_idx] = exp_sum;
    }
...
#pragma unroll
    ...
    if (part_start < ctx_len) {
        ...
        for (int block_idx = cache_start_block + warp_idx;
            ...
            const int physical_block_offset = (lane % NUM_V_VECS_PER_ROW) * V_VEC_SIZE;
            const int token_idx = block_idx * BLOCK_SIZE + physical_block_offset;
            const int logit_offset = token_idx - part_start;

            L_vec logits_vec;
            vllm::from_float(logits_vec,
                       *reinterpret_cast<Float_L_vec *>(logits + logit_offset));
...
#pragma unroll
            for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
                ...
                if (row_idx < HEAD_SIZE) {
                    ...
                    if (block_idx == cache_end_block - 1) {
                        scalar_t *v_vec_ptr = reinterpret_cast<scalar_t *>(&v_vec);
#pragma unroll
                        for (int j = 0; j < V_VEC_SIZE; j++) {
                            v_vec_ptr[j] =
    ...
    // V Phase 2: New tokens from input
    if (new_end_global > new_start_global) {
        const int new_start = new_start_global - ctx_len;
        const int new_end = new_end_global - ctx_len;
        const int new_start_block = new_start / BLOCK_SIZE;
        ...
        for (int block_idx = new_start_block + warp_idx;
            ...
            const int logit_base = ctx_len + base_new_idx - part_start;
            L_vec logits_vec;
            vllm::from_float(logits_vec,
                       *reinterpret_cast<Float_L_vec *>(logits + logit_base + physical_block_offset));
...
#pragma unroll
    ...
    // Write partial output to tmp_out
    if (warp_idx == 0) {
        scalar_t *out_ptr =
            tmp_out + token_flat_idx * num_heads * max_num_partitions * HEAD_SIZE
...
/// Reduce kernel for context attention v2.
///
/// Grid: (num_heads, total_new_tokens)
/// Merges partition results via log-sum-exp.
template <typename scalar_t, int HEAD_SIZE, int NUM_THREADS, int PARTITION_SIZE>
__global__ void context_attention_fwd_v2_reduce_kernel(
    scalar_t *__restrict__ out,         // [total_new_tokens, num_heads, head_size]
    const float *__restrict__ exp_sums,       // [total_new_tokens, num_heads, max_num_partitions]
    const float *__restrict__ max_logits_arr, // [total_new_tokens, num_heads, max_num_partitions]
    const scalar_t *__restrict__ tmp_out,     // [total_new_tokens, num_heads, max_num_partitions, head_size]
    const int *__restrict__ context_lens,
    const int *__restrict__ query_start_locs,
    const int *__restrict__ seq_ids,
    const int max_num_partitions,
    ...
    // Compute per-token total_kv_len to find num_partitions
    const int batch_idx = seq_ids[token_flat_idx];
    const int q_start = query_start_locs[batch_idx];
    const int q_local_idx = token_flat_idx - q_start;
    const int ctx_len = context_lens[batch_idx];
    const int total_kv_len = ctx_len + q_local_idx + 1;
    const int num_partitions = CTX_DIVIDE_ROUND_UP(total_kv_len, PARTITION_SIZE);

    if (num_partitions == 1 && sinks == nullptr) {
        // No need to reduce. Just copy tmp_out to out.
        scalar_t *out_ptr =
            out + token_flat_idx * num_heads * HEAD_SIZE + head_idx * HEAD_SIZE;
    ...
    constexpr int NUM_WARPS = NUM_THREADS / CTX_WARP_SIZE;
    const int warp_idx = threadIdx.x / CTX_WARP_SIZE;
    const int lane = threadIdx.x % CTX_WARP_SIZE;
    ...
    const float *max_logits_ptr = max_logits_arr
        + token_flat_idx * num_heads * max_num_partitions
...
#pragma unroll
    ...
    if (lane == 0) {
        red_smem[warp_idx] = global_exp_sum;
    }
    __syncthreads();
    if (lane < NUM_WARPS) {
        global_exp_sum = red_smem[lane];
    }
#pragma unroll
    for (int mask = NUM_WARPS / 2; mask >= 1; mask /= 2) {
        global_exp_sum += VLLM_SHFL_XOR_SYNC(global_exp_sum, mask);
    }
    global_exp_sum = VLLM_SHFL_SYNC(global_exp_sum, 0);

    // Include sink in global exp sum
    if (sinks != nullptr) {
...
#pragma unroll
    for (int i = threadIdx.x; i < HEAD_SIZE; i += NUM_THREADS) {
        ...
        for (int j = 0; j < num_partitions; ++j) {
            acc += vllm::to_float(tmp_out_ptr[j * HEAD_SIZE + i]) * shared_exp_sums[j]
                   * inv_global_exp_sum;
        }
        vllm::from_float(out_ptr[i], acc);
    }
}
...
#define LAUNCH_CTX_ATTN_V2(HEAD_SIZE)                                          \
  VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                        \
      ((void *)vllm::context_attention_fwd_v2_kernel<T, CACHE_T, KV_DT,       \
                                                     HEAD_SIZE, BLOCK_SIZE,    \
                                                     NUM_THREADS, PARTITION_SIZE>), \
      shared_mem_size);                                                        \
  vllm::context_attention_fwd_v2_kernel<T, CACHE_T, KV_DT, HEAD_SIZE,         \
                                        BLOCK_SIZE, NUM_THREADS, PARTITION_SIZE> \
      <<<grid, block, shared_mem_size, stream>>>(                              \
          exp_sums, max_logits_buf,                                            \
          reinterpret_cast<T *>(tmp_out),                                      \
  ...
  vllm::context_attention_fwd_v2_reduce_kernel<T, HEAD_SIZE, NUM_THREADS,      \
                                               PARTITION_SIZE>                 \
      <<<reduce_grid, block, reduce_shared_mem_size, stream>>>(                \
          reinterpret_cast<T *>(out), exp_sums, max_logits_buf,                \
          reinterpret_cast<const T *>(tmp_out),                                \
          context_lens, query_start_locs, seq_ids,                             \
          max_num_partitions, sinks);

template <typename T, typename CACHE_T, vllm::Fp8KVCacheDataType KV_DT,
          int BLOCK_SIZE, int NUM_THREADS = 128, int PARTITION_SIZE = 512>
inline void context_attention_fwd_v2_launcher(
    void *out, float *exp_sums, float *max_logits_buf, void *tmp_out,
    ...
    const float *k_scale, const float *v_scale,
    int sliding_window, const float *sinks,
    int max_total_kv_len, int max_num_partitions) {
    ...
    dim3 grid(num_heads, total_new_tokens, max_num_partitions);
    dim3 block(NUM_THREADS);

    dim3 reduce_grid(num_heads, total_new_tokens);
    int reduce_shared_mem_size = 2 * max_num_partitions * sizeof(float);

    switch (head_size) {
    case 64:
        LAUNCH_CTX_ATTN_V2(64);
        break;
    case 80:
        LAUNCH_CTX_ATTN_V2(80);
        break;
    case 96:
        LAUNCH_CTX_ATTN_V2(96);
        break;
    case 112:
        LAUNCH_CTX_ATTN_V2(112);
        break;
    case 128:
        LAUNCH_CTX_ATTN_V2(128);
        break;
    case 192:
        LAUNCH_CTX_ATTN_V2(192);
        break;
    case 256:
        LAUNCH_CTX_ATTN_V2(256);
        break;
    default:
        break;
    }
...
#define CALL_CTX_ATTN_V2_LAUNCHER(T, CACHE_T, KV_DT, BLOCK_SIZE)              \
  context_attention_fwd_v2_launcher<T, CACHE_T, KV_DT, BLOCK_SIZE>(           \
      out, exp_sums, max_logits_buf, tmp_out,                                  \
      query, key, value, key_cache, value_cache, num_kv_heads, scale,          \
      block_tables, context_lens, query_lens, query_start_locs, seq_ids,       \
      max_num_blocks_per_seq, total_new_tokens, num_heads, head_size,          \
      kv_block_stride, kv_head_stride, stream, k_scale, v_scale,              \
      sliding_window, sinks, max_total_kv_len, max_num_partitions);

#define CALL_CTX_ATTN_V2_LAUNCHER_BLOCK_SIZE(T, CACHE_T, KV_DT)               \
  switch (block_size) {                                                        \
  case 8:                                                                      \
    CALL_CTX_ATTN_V2_LAUNCHER(T, CACHE_T, KV_DT, 8);                          \
    break;                                                                     \
  case 16:                                                                     \
    CALL_CTX_ATTN_V2_LAUNCHER(T, CACHE_T, KV_DT, 16);                         \
    break;                                                                     \
  case 32:                                                                     \
    CALL_CTX_ATTN_V2_LAUNCHER(T, CACHE_T, KV_DT, 32);                         \
    break;                                                                     \
  default:                                                                     \
    break;                                                                     \
  }

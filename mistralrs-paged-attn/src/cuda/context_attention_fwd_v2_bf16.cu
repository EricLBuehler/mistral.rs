...
extern "C" void context_attention_fwd_v2_bf16(
    void *out,              // [total_new_tokens, num_heads, head_size]
    float *exp_sums,        // [total_new_tokens, num_heads, max_num_partitions]
    float *max_logits_buf,  // [total_new_tokens, num_heads, max_num_partitions]
    void *tmp_out,          // [total_new_tokens, num_heads, max_num_partitions, head_size]
    ...
    int32_t max_total_kv_len, int32_t max_num_partitions) {

  if (cache_dtype == 3) {
    CALL_CTX_ATTN_V2_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, uint8_t,
                                          vllm::Fp8KVCacheDataType::kFp8E4M3);
  } else {
    CALL_CTX_ATTN_V2_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, __nv_bfloat16,

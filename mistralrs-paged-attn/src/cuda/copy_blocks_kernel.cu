...
extern "C" void copy_blocks_f32(int64_t *key_cache_ptrs,
                                ...
                                int numel_per_block, int64_t stream_) {
  cudaStream_t stream = (cudaStream_t)stream_;
  if (numel_per_block > 1024) {
    numel_per_block = 1024;
  }
  copy_blocks_kernel_f32<<<dim3(num_layers, num_pairs, 1), numel_per_block, 0,
...
extern "C" void copy_blocks_f16(int64_t *key_cache_ptrs,
  ...
  copy_blocks_kernel_f16<<<dim3(num_layers, num_pairs, 1), numel_per_block, 0,
                           stream>>>(key_cache_ptrs, value_cache_ptrs,
                                     block_mapping, numel_per_block);
}
...
extern "C" void copy_blocks_bf16(int64_t *key_cache_ptrs,
  ...
  copy_blocks_kernel_bf16<<<dim3(num_layers, num_pairs, 1), numel_per_block, 0,
                            stream>>>(key_cache_ptrs, value_cache_ptrs,
                                      block_mapping, numel_per_block);
}
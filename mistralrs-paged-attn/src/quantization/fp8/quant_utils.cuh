...
__inline__ __device__ uint16_t scaled_vec_conversion<uint16_t, uint8_t>(
  ...
  return __float2half(__half2float(tmp.x) * scale);
}
...
__inline__ __device__ uint32_t scaled_vec_conversion<uint32_t, uint16_t>(
  ...
  __half2_raw res = __nv_cvt_fp8x2_to_halfraw2(a, fp8_type);
  tmp.u16[0] = __float2half(__half2float(res.x) * scale);
  tmp.u16[1] = __float2half(__half2float(res.y) * scale);
  return tmp.u32;
}
...
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, uint16_t>(
    ...
    const __nv_fp8_interpretation_t fp8_type) {
  __nv_fp8_storage_t res =
      __nv_cvt_float_to_fp8(__half2float(a) / scale, __NV_SATFINITE, fp8_type);
  return (uint8_t)res;
}
...
// Src dtype 0 => f16; 1 => bf16; 2 => f32
// KV dtype 0 => same as src dtype; 1 => fp8e4m3

...
pub fn run_cache_prune() -> Result<()> {
    let hub_dir = hf_hub_dir()?;

    println!();
    println!("Pruning HuggingFace Cache");
    println!("-------------------------");
    println!();
    ...
    // Collect items to prune (we need to collect first to avoid iterator invalidation)
    let mut items_to_prune = Vec::new();

    for entry in walkdir::WalkDir::new(&hub_dir)
        .into_iter()
        .filter_map(|e| e.ok())
    {
        let name = entry.file_name().to_string_lossy();
        if name.ends_with(".incomplete") || name.starts_with(".tmp") {
            items_to_prune.push(entry.path().to_path_buf());
        }
    }

    // Now prune (in reverse order to handle nested directories)
    items_to_prune.sort();
    items_to_prune.reverse();

    for path in items_to_prune {

...
<header class="flex h-12 shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-4 dark:border-gray-800 dark:bg-gray-950">
  ...
  <div class="flex min-w-0 items-center gap-2">
    {#if modelStore.selectedModel}
      <span class="truncate text-sm font-medium">{modelStore.selectedModel}</span>
      <span class="shrink-0 rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
        {modelStore.selectedModelKind}
      </span>
    {:else}
      <span class="text-sm text-gray-400">No model selected</span>
    {/if}
  </div>

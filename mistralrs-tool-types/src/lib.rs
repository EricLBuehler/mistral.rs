...
#[cfg(test)]
mod tests {
    use super::{CalledFunction, Function, Tool, ToolType};
    use serde_json::json;
    use std::collections::HashMap;
    ...
    fn tool_schema_round_trips() {
        ...
        assert_eq!(decoded.function.strict, Some(true));
    }

    #[test]
    fn function_accepts_arguments_alias() {
        ...
        .unwrap();
        assert!(function.parameters.unwrap().contains_key("city"));
    }

    #[test]
    fn called_function_round_trips() {
        let called = CalledFunction {
            name: "weather".to_string(),
            arguments: "{\"city\":\"Paris\"}".to_string(),
        };
        let value = serde_json::to_value(&called).unwrap();
        let decoded: CalledFunction = serde_json::from_value(value).unwrap();
        assert_eq!(decoded.name, called.name);
        assert_eq!(decoded.arguments, called.arguments);
    }
}

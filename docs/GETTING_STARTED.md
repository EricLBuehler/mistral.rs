...
**Recommended default:** Google Gemma 4 (multimodal, strong tool calling). Text-only models like `Qwen/Qwen3-4B` work the same way.
...
```bash
mistralrs serve --isq 4 -m google/gemma-4-E4B-it
```
...
```bash
mistralrs serve --ui --isq 4 -m google/gemma-4-E4B-it
...
```bash
mistralrs tune -m google/gemma-4-E4B-it --emit-config config.toml

...
`--quant 4` prefers a prebuilt [UQFF (Universal Quantized File Format)](/reference/uqff-format/) quantization from `mistralrs-community` when one is published, otherwise quantizes the weights as they load (in-situ). Omit it to load native precision (BF16 for Qwen3-4B, about 8 GB). See [Quantization](/guides/quantization/quantize-a-model/) for the decision guide.

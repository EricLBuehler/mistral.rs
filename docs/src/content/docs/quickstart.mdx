...
Prebuilt binaries do not require Rust or a CUDA toolkit. Installer-managed binaries install to `~/.mistralrs`, with `mistralrs` symlinked into `~/.local/bin` on Unix and launched from `%USERPROFILE%\.local\bin` on Windows. If needed, the installer adds that directory to your shell environment.

Source builds use Cargo as a staging step, then install to the same managed location.
...
| Linux x86_64 + NVIDIA GPU | prebuilt, matched to GPU and driver | CUDA, flash-attn, cuTile on supported CUDA/SM pairs |
| Linux aarch64 + NVIDIA GPU (Grace: GH200/GB200/GB10) | prebuilt, matched to GPU and driver | CUDA, flash-attn, cuTile on supported CUDA/SM pairs |
...
| CUDA 13.1+ on Blackwell / `sm100`, `sm120`, `sm121` | `cuda131` |
| CUDA 13.0+ | `cuda130` |
...
- `MISTRALRS_INSTALL_TAG=v0.8.22` installs a specific release instead of the latest (downloads that release's prebuilt, or builds that git tag from source).

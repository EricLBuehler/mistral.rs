...
Prebuilt binaries do not require Rust or a CUDA toolkit for standard acceleration. Installer-managed binaries install to `~/.mistralrs`, with `mistralrs` symlinked into `~/.local/bin` on Unix and launched from `%USERPROFILE%\.local\bin` on Windows. If needed, the installer adds that directory to your shell environment. Optional cuTile acceleration requires NVIDIA's separately installed `tileiras` tool. See [cuTile setup](/mistral.rs/developer/moe-backends/) for the official package commands.
...
If no artifact matches the GPU or driver, the installer builds from source. See [hardware support](/mistral.rs/reference/hardware-support/) for the full matrix.
...
Already have a GGUF? Use an exact local file for chat, or select a published quantization and serve
it directly:
...
The filename selects GGUF automatically. From a recognized GGUF repository, mistral.rs discovers
the tokenizer, configuration, chat template, and multimodal projector; explicit flags remain
available as overrides. See [Run GGUF models](/mistral.rs/guides/models/run-gguf/) for loading and
serving workflows, or [GGUF support](/mistral.rs/reference/gguf-support/) for the compatibility
matrix.
...
`with_auto_isq(IsqBits::Four)` is the in-process equivalent of `--isq 4`. See the [Rust SDK guide](/mistral.rs/guides/rust/getting-started/) for streaming, multimodal input, and sampling options.

## Next steps

- [Run models](/mistral.rs/guides/models/run-any-model/): Hugging Face IDs, local files, GGUF, and quantization flags.
- [Run GGUF models](/mistral.rs/guides/models/run-gguf/): local files, Hub artifacts, multimodal input, LoRA, and overrides.
...
- [Agents & tools](/mistral.rs/guides/agents/): tool calling, web search, code execution, shell, OpenAI-compatible Skills, and MCP (Model Context Protocol).

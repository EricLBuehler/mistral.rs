...
Prebuilt binaries are self-contained (no CUDA toolkit or Rust needed) and install to `~/.mistralrs`, with `mistralrs` symlinked into `~/.local/bin` (`%USERPROFILE%\.local\bin` on Windows). If that directory is not on your `PATH`, the installer adds it persistently through a rustup-style env file on Unix or the user PATH on Windows. A source build instead installs to `~/.cargo/bin/mistralrs`, which is on `PATH` after a new shell or `source "$HOME/.cargo/env"`.
...
- `MISTRALRS_INSTALL_TAG=v0.8.5` installs a specific release instead of the latest (downloads that release's prebuilt, or builds that git tag from source).

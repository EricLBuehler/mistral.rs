...
Prebuilt binaries do not require Rust or a CUDA toolkit. They install to `~/.mistralrs`, with `mistralrs` symlinked into `~/.local/bin` (`%USERPROFILE%\.local\bin` on Windows). If needed, the installer adds that directory to your shell environment.

Source builds install through Cargo to `~/.cargo/bin/mistralrs`.
...
| Linux x86_64 + NVIDIA GPU | prebuilt, matched to GPU and driver | CUDA, flash-attn, cuTile on CUDA 13.1+ |
| Linux aarch64 + NVIDIA GPU (Grace: GH200/GB200/GB10) | prebuilt, matched to GPU and driver | CUDA, flash-attn, cuTile on CUDA 13.1+ |
...
|---|---|
| CUDA 13.1+ | `cuda131` |
| CUDA 12.9 or 13.0 on GB10 / `sm121` | `cuda129` |
| CUDA 12.8 to 13.0 on other GPUs | `cuda128` |
...
- `MISTRALRS_INSTALL_TAG=v0.8.15` installs a specific release instead of the latest (downloads that release's prebuilt, or builds that git tag from source).

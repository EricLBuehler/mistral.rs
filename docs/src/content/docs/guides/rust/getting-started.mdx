...
`ModelBuilder::build()` performs all model loading and is expensive. Call it once at startup. `Model` does not implement `Clone`; its methods take `&self` and are safe to call concurrently, so share one instance by reference or wrap it in `Arc<Model>` to move it across spawned tasks.

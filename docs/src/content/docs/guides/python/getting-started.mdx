...
Look up your GPU's compute capability in [hardware support](/mistral.rs/reference/hardware-support/). CUDA wheels bundle the CUDA runtime but omit cuTile, so they use the native CUDA routed-LoRA kernel and the CUTLASS MoE backend. For cuTile, use a [cutile-capable CLI binary](/mistral.rs/quickstart/) and install NVIDIA's official `tileiras` package as described in [MoE expert backends](/mistral.rs/developer/moe-backends/).

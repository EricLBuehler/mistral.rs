...
| Loader | GPU memory target | Availability | Notes |
|---|---|---|---|
| `Flux` | about 33 GB | CLI, server, SDK | Fully loaded path. Fastest. |
| `FluxOffloaded` | about 4 GB | SDK only | CPU offload path. Useful when the full model does not fit. |

...
Most models need nothing beyond `mistralrs run -m <id>` (see [Run models](/guides/models/run-any-model/)). This page collects the per-family exceptions. The full architecture inventory is in the [supported models reference](/reference/supported-models/).
...
Qwen3 also publishes FP8 pre-quantized checkpoints; pass the FP8 model ID directly when you want those weights instead of runtime [ISQ (in-situ quantization)](/reference/quantization-types/).

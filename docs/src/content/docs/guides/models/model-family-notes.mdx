...
HYV3 checkpoints load through `HYV3ForCausalLM`. The loader covers the main causal LM path used by `tencent/Hy3` BF16 and `tencent/Hy-MT2-30B-A3B`, including Q/K RMSNorm, sigmoid MoE routing with expert bias, the shared expert, and fp32 output head projection when requested by the checkpoint. HYV3 tool calls use the model-native `<tool_calls...>` / `<arg_key...>` tag format from the checkpoint chat template.

`tencent/Hy3` also ships one MTP layer (`num_nextn_predict_layers = 1`) for speculative decoding. Standard generation does not require that layer; the current HYV3 loader ignores the extra MTP weights. `tencent/Hy3-FP8` uses FP8 scale tensors and requires FP8 checkpoint support before it can be loaded as published.

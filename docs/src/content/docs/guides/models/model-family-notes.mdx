...
Most models need nothing beyond `mistralrs run -m <id>` (see [Run models](/mistral.rs/guides/models/run-any-model/)). This page collects the per-family exceptions. The full architecture inventory is in the [supported models reference](/mistral.rs/reference/supported-models/).
...
The model's ATEM chat template is detected automatically. Tool calling supports automatic, required, named, and parallel calls through the same HTTP, Python, Rust, and agent APIs as other tool-capable models. `reasoning_effort` selects the template's `low`, `medium`, or `high` reasoning strength.

ISQ, UQFF, paged attention, prefix caching, tensor parallelism, and text-backbone LoRA use the standard multimodal model paths. For GGUF-specific requirements and its image-only projector limitation, see [GGUF compatibility](/mistral.rs/reference/gguf-support/).

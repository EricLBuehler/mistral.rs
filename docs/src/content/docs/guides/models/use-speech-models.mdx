---
title: Speech models
description: Voxtral for speech transcription, Dia for text-to-speech.
---
...
- **Voxtral**: multimodal model accepting audio input. Used for speech transcription through `/v1/chat/completions`.
...
Voxtral fits the multimodal chat shape: audio is an input content part and the response is its
transcription. Voxtral Realtime does not use the accompanying text instruction to select other
audio tasks such as summarization or speaker analysis.

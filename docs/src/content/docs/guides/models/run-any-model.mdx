...
`--quant <level>` is the quantization front door. Numeric levels (`2`, `3`, `4`, `5`, `6`, `8`)
work across model sources. Standard model repositories also accept
[ISQ (in-situ quantization)](/mistral.rs/reference/quantization-types/) names such as `q4k` and
`afq8`; GGUF repositories accept supported Q/K artifact names such as `q4k`. See
[GGUF compatibility](/mistral.rs/reference/gguf-support/) for the available storage formats. A GGUF
repository supplies its matching GGUF variant. Other model repositories prefer a prebuilt
[UQFF (Universal Quantized File Format)](/mistral.rs/reference/uqff-format/) from
`mistralrs-community/<model-name>-UQFF` and fall back to ISQ.
...
Use the same GGUF for local chat, continuous batching, or an OpenAI-compatible API:
...
The `.gguf` filename selects the format automatically. When loading from a recognized GGUF
repository, mistral.rs discovers the assets needed for chat and multimodal input; `--tok-model-id`
and `--mmproj` remain available as explicit overrides. The same command shape works with `run`,
`serve`, and `bench`.

See [Run GGUF models](/mistral.rs/guides/models/run-gguf/) for exact Hub files, automatic
quantization selection, multimodal input, LoRA, ISQ, offline loading, and Python and Rust examples.
The [GGUF support reference](/mistral.rs/reference/gguf-support/) lists compatible architectures,

...
`--quant <level>` is the quantization front door. With a numeric level (`2`, `3`, `4`, `5`, `6`, `8`) or an ISQ name (`q4k`, `afq8`, ...), it first looks for a prebuilt UQFF at `mistralrs-community/<model-name>-UQFF` and downloads the matching file; if no UQFF repo or matching shard exists (or the model is a local path), it falls back to in-situ quantization (ISQ) at that level.

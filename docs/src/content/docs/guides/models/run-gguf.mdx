...
available. Model sampling defaults declared under `general.sampling.*` are used when the repository
has no `generation_config.json`; see [Sampling](/guides/customize/sampling/). It also uses the selected repository or the file's base-model information to find

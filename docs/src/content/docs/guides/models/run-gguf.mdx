...
matches the model dtype. The direct local shorthand, `-f /path/model.gguf` with `-m` omitted, also
selects an unambiguous projector stored beside the model. Model files, projector files, tokenizer
assets, and processor assets can therefore be loaded with one repository command:
...
| `... --mmproj mmproj-BF16.gguf` | Omit for a recognized GGUF repository or the direct local shorthand with an adjacent, unambiguous projector. |

...
Vectors are not L2-normalized; mistral.rs returns them as the model produces them. If you compute cosine similarity as a raw dot product, normalize on the client first.

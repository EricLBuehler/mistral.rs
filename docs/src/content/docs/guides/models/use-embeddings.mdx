...
mistral.rs handles steps 1, 2, and (with a reranker) 4. The rest is the vector store and application logic. The [web search guide](/guides/agents/web-search/) covers using embeddings to rerank search results within an agent.

...
Legacy static GGUF LoRA remains available for Llama, Mistral3, and Phi3 through `--legacy-lora` with `--legacy-lora-order`. GGML uses the same legacy options. X-LoRA remains supported through `--xlora` with `--xlora-order`. Multimodal models support dynamic language-model LoRA, but not legacy LoRA or X-LoRA. Neither legacy mode nor X-LoRA supports dynamic per-request selection.
...
|---|---|---|---|
| Dynamic LoRA | `--lora`, `Which.Lora`, `LoraModelBuilder`, `Which.GGUF`, `GgufModelBuilder` | Zero or one selected adapter | Yes |
| Legacy GGUF/GGML LoRA | `--legacy-lora` with `--legacy-lora-order`, `Which.LoraGGUF`, `Which.LoraGGML` | One static adapter configuration | No |
| X-LoRA | `--xlora` with `--xlora-order`, `Which.XLora`, `Which.XLoraGGUF`, `Which.XLoraGGML` | Learned per-token mixture | No |

## Legacy raw LoRA

Use dynamic LoRA for a supported GGUF. Legacy mode remains available for
GGML and for compatible static GGUF adapter configurations on Llama, Mistral3, and Phi3.
...
The same dynamic runtime works with supported GGUF models:
...
GGUF supports preloading and request routing on every surface. Live load, list, replace, and unload
use the server endpoints or Python and Rust APIs described below.
...
For a GGUF repository, keep the same model selection flags:

```bash
mistralrs serve -m <text-gguf-repo> --quant 4 --enable-lora
```

:::caution[Multimodal adapter boundaries]
Multimodal models accept adapters for the language model. Vision, audio, and projector adapters are
not supported. GGUF repositories still select a companion projector automatically; `--mmproj`
overrides that choice. Legacy LoRA and X-LoRA remain unavailable with multimodal GGUF. GGML uses
`--legacy-lora` with `--legacy-lora-order`; legacy GGUF mode remains available for Llama, Mistral3,
and Phi3.
:::
...
Python exposes the same limits on `Which.Lora` and `Which.GGUF`. Rust uses
...
For a GGUF base, select the quantized file and pass the same adapter specification:
...
For multimodal GGUF, add `mmproj_filename="<mmproj.gguf>"` in Python or
`.with_mmproj_files(vec!["<mmproj.gguf>"])` in Rust. The adapter remains selectable for text and
media requests.

Use `adapters=[]` in Python or `.with_lora()` in Rust to enable an initially empty runtime. In
Python, setting any LoRA limit to a non-default value also enables an empty runtime.
...
Routed expert LoRA is available for these model families:
...
The default `auto` model selection is recommended for safetensors multimodal LoRA. Image options
such as `--max-edge`, `--max-num-images`, and `--max-image-length` remain active when LoRA is
enabled. Python and Rust expose multimodal GGUF LoRA through `Which.GGUF` and
`GgufModelBuilder`.
...
| Request APIs | Chat Completions, Completions, and Responses, including multimodal requests supported by the selected model. Not Embeddings, Anthropic Messages, diffusion, or speech requests. |
...
| GGUF/GGML | Supported GGUF models use dynamic LoRA, including language-model adapters with multimodal GGUF. GGML uses legacy mode without per-request selection or hot-loading; legacy GGUF mode remains available for Llama, Mistral3, and Phi3. |
...
| Raw GGUF/GGML `--lora <source>` | For a supported GGUF, use `--lora alias=source` and select the alias dynamically. For GGML, or to retain a compatible static Llama/Mistral3/Phi3 configuration, use `--legacy-lora <source> --legacy-lora-order <ordering-file.json>`; the [ordering JSON](#legacy-raw-lora) is required. |
...
| Raw GGUF/GGML TOML LoRA fields | For a supported GGUF, use the structured dynamic `lora` entries under `[models.adapter]`. Use `legacy_lora` with `legacy_lora_order` for GGML or a compatible static Llama, Mistral3, or Phi3 GGUF configuration. |
...
| Multimodal LoRA | Compatible language-model adapters work through the CLI and server. Python `Which.GGUF` and Rust `GgufModelBuilder` expose the same support for multimodal GGUF. Vision, audio, and projector adapters are unsupported. |

...
Raw GGUF/GGML LoRA and X-LoRA remain supported through their existing configuration options. They do not support dynamic per-request selection.
...
|---|---:|---|
| `--lora-max-adapters` | 16 | Maximum number of loaded adapter versions. |
...
This list is narrower than general MoE model support. Granite MoE and multimodal MoE pipelines other than Qwen3.5/3.6 MoE are not supported. AnyMoE cannot be combined with dynamic per-request LoRA. Tensor parallelism supports preloaded adapters, but expert parallelism does not support dynamic routed LoRA.
...
`GET /v1/lora_adapters` is always available, including when runtime mutation is disabled. It reports loaded adapters, generation IDs, capacity, and configured limits. Source paths are included only while mutation is enabled. In a multi-model server, pass `model` in load and unload request bodies and `?model=<model-id>` when listing.
...
Loads are serialized and non-queueing. A concurrent HTTP load returns 429 with code `lora_load_busy`; retry it with bounded backoff and jitter. A load can complete after the client disconnects, so query the list endpoint to verify the published generation after a timeout. Retrying the same adapter load returns the same generation.

Tensor-parallel deployments support preloaded adapters but not live load or unload operations.

### Enable HTTP mutation

The read-only list endpoint is always available. Runtime load and unload are available only when `MISTRALRS_ALLOW_RUNTIME_LORA_UPDATING` is enabled. `MISTRALRS_LORA_ADAPTER_ROOT` restricts `lora_path` to a canonical directory tree:
...
| Base weights | Plain safetensors, in-situ quantization, and UQFF. |

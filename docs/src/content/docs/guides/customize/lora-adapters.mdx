...
| Legacy LoRA | GGML and compatible static GGUF configurations | One static adapter configuration | No |
| X-LoRA | Compatible plain, GGUF, and GGML text models | Learned per-token mixture | No |

For GGUF file and projector selection, see [Run GGUF models](/mistral.rs/guides/models/run-gguf/). The [GGUF support reference](/mistral.rs/reference/gguf-support/) summarizes adapter and multimodal boundaries.
...
The same dynamic runtime works with GGUF:
...
GGUF uses the same aliases, request routing, and lifecycle APIs described below. Model files and companion projectors follow the normal [GGUF selection rules](/mistral.rs/guides/models/run-gguf/).
...
For a GGUF base, select an exact file and pass the same adapter specification. The [GGUF guide](/mistral.rs/guides/models/run-gguf/) covers automatic selection from the CLI.
...
The same builders support multimodal GGUF. Configure the model and companion assets exactly as you would without LoRA; the language-model adapter remains selectable for text and media requests.
...
| GGUF and GGML | GGUF supports dynamic LoRA for compatible language-model adapters. GGML uses legacy mode. See [GGUF support](/mistral.rs/reference/gguf-support/) for legacy and multimodal boundaries. |
...
Use dynamic LoRA for GGUF whenever possible. Legacy mode remains available for GGML and the static GGUF configurations listed in the [GGUF support reference](/mistral.rs/reference/gguf-support/).

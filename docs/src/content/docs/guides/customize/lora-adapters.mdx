...
Raw GGUF/GGML LoRA remains supported for text models through legacy mode, which requires `--legacy-lora` and `--legacy-lora-order`. X-LoRA remains supported through `--xlora` with `--xlora-order`. Multimodal GGUF does not currently support LoRA or X-LoRA. Neither mode supports dynamic per-request selection.

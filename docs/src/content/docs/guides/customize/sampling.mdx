...
Fields a request leaves unset fall back to the model's `generation_config.json` (temperature, top-k, top-p, min-p, repetition penalty). A GGUF model with no `generation_config.json` instead falls back to the sampling keys in its own metadata: `general.sampling.temp`, `general.sampling.top_k`, `general.sampling.top_p`, `general.sampling.min_p`, and `general.sampling.penalty_repeat` (the repetition penalty). A `generation_config.json` takes precedence when both exist. Without such defaults the engine samples at temperature 1.0 with no filters. Send `temperature: 0` (or `top_k: 1`) for greedy decoding.
...
Until overridden, interactive mode seeds its sampling from the model's `generation_config.json` or GGUF sampling metadata when present, else temperature 0.8, top-k 40, top-p 0.95, min-p 0.05.

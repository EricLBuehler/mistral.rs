...
Generation ends at the model's EOS tokens, at any `stop` string or token the request supplies, or at
the token limit (`max_tokens`), whichever comes first.

Set `ignore_eos: true` to keep generating past the model's EOS tokens. Explicit stops and the token
limit still apply, so pair it with `max_tokens` or a `stop` value to bound the response. It is useful
for base (non-instruct) models that emit EOS early, and for measuring raw decode throughput.

...
Highest priority first: `--jinja-explicit`; then a multimodal `processor_config.json` template; then `--chat-template`; then the model repo's own template; then GGUF metadata as a last resort.

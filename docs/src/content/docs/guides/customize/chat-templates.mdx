...
The engine settles on a template in this order:

1. `--chat-template <file>`, if given. The file replaces whatever the model repo provides and must end in `.json` or `.jinja`.
2. Otherwise, `chat_template.jinja` in the model repo, if present.
3. Otherwise, the `chat_template` field of the repo's `tokenizer_config.json`.
4. If still empty, the repo's standalone `chat_template.json` (some newer models ship the template as this separate file).
5. `--jinja-explicit <file.jinja>`, if given, overrides the result of 1-4. The value must be a path to a `.jinja` file.
6. For multimodal models whose `processor_config.json` contains a `chat_template`, that template takes precedence over everything above.
7. GGUF models: the template embedded in GGUF metadata is used only when none of the other sources produced one.
...
- `date_string`: the current UTC date.

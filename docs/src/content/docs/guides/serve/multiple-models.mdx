...
`quant = "4"` is the front door (same as `--quant 4`): it prefers a prebuilt UQFF and falls back to 4-bit [ISQ (in-situ quantization)](/mistral.rs/reference/quantization-types/) at load time. Use `isq = "4"` (same as `--isq 4`) for explicit ISQ; drop both to load full precision.

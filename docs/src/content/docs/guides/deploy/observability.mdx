...
| Metric | Type | Labels | Meaning |
|---|---|---|---|
| `mistralrs_time_to_first_token_seconds` | histogram | `method`, `path`, `model`, `status` | Time from request start to the first SSE content frame. Includes queueing and prefill. |
| `mistralrs_inter_token_latency_seconds` | histogram | `method`, `path`, `model`, `status` | Gaps between consecutive SSE content frames. One sample per frame after the first. |

SSE keep-alive ping frames carry no tokens and are excluded from both. Long decode stalls show up as high ITL quantiles, which a per-request average would hide. Both histograms use the same bucket boundaries as vLLM's exporter; the existing `http_request_*` histograms keep their own bucket lists.
...
| `mistralrs_prefix_cache_evictions_total` | counter | - | Cached KV prefix entries evicted to free memory. |
...
| `mistralrs_sequences_completed_total` | counter | `reason` | Sequences that reached a terminal state. `reason` is one of `stop`, `length`, `canceled`, `generated_image`, `generated_speech`, `tool_calls`. |
...
histogram_quantile(0.95,
  sum(rate(mistralrs_inter_token_latency_seconds_bucket[5m]
    {path="/v1/chat/completions"})) by (le))

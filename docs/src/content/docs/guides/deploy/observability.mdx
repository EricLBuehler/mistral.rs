...
| `mistralrs_encoder_cache_evictions_total` | counter | `reason` | Encoder outputs removed because of entry, programmatic logical-byte, or shape constraints. |
| `mistralrs_encoder_cache_insert_rejections_total` | counter | `reason` | Encoder outputs rejected because one entry exceeds a programmatic logical-byte budget. |
| `mistralrs_encoder_cache_resident_entries` | gauge | - | Encoder outputs resident across all encoder caches in the process. |
| `mistralrs_encoder_cache_resident_logical_bytes` | gauge | - | Logical tensor payload bytes resident across all encoder caches in the process. Tensor views can share physical storage. |

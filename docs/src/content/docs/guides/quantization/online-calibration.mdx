...
An exact compatible GGUF source works too:
...
- For best quality, start from a high-precision checkpoint. When exact source weights are not
  available for a layer, mistral.rs uses its resident weights and logs a warning.

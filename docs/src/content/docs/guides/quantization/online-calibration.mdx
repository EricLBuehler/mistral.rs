...
[ISQ (in-situ quantization)](/mistral.rs/reference/quantization-types/), builds an
[importance matrix](/mistral.rs/guides/quantization/quantize-a-model/#imatrix) from that traffic,
...
- The model must have been loaded with `--isq` from a supported safetensors or GGUF source;
  ...
  [`--from-uqff`](/mistral.rs/guides/quantization/uqff/).

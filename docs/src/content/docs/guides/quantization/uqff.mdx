...
AFQ variants automatically promote token embeddings and output heads: AFQ2/3/4 uses AFQ6 for
those tensors, while AFQ6/8 uses AFQ8. The filename still names the default model type, and
`uqff_report.json` records the effective type for each tensor. Explicit topology pins take
precedence.

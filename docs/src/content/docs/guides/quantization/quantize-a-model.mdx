...
compatible GGUF inputs selected with either `-f` or `-m <repo> --quant <artifact>`.
...
layer to a new type in place. The model must have been loaded with ISQ; this includes compatible
GGUF sources loaded with `--isq`, but not legacy GGUF or GGML models:
...
- **GGUF**: load a local file directly with `-f <file.gguf>`, or select a published variant with
  ...
  intentionally want to requantize it.

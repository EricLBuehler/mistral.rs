...
`--mtp-n-predict` defaults to `min(block_size - 1, 7)` for DFlash drafters; the checkpoint's block
size is the upper bound. The drafter's projection weights are requantized to the target's in-situ
quantization type at load (override with the `MISTRALRS_DFLASH_ISQ` environment variable, e.g.
`q6k` or `none` for bf16).
...
of its training distribution. For thinking-heavy workloads the built-in `--mtp` head can be the
better choice. As everywhere in this engine, drafts are verified by the target: greedy output is

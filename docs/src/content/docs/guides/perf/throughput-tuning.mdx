...
Two limits control how new prompts share the GPU with active decodes:

- `--max-num-batched-tokens` bounds the total prompt tokens in a scheduler batch (default 4096).
  The budget is divided across the admitted prompt sequences, keeping peak activation memory
  stable as concurrency grows.
...
Measure both output throughput and latency percentiles when changing either value. A larger token
budget can improve prefill throughput while increasing decode latency, especially for long prompts.

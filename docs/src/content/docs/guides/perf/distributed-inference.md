...
`-n`/`--device-layers` assigns layer counts to devices, format `ORD:NUM;...`:
...
For mixed GPU memory sizes, manually set `--device-layers`; the automatic split does not optimize for heterogeneous memory or PCIe/NVLink topology. Multi-socket servers with GPUs on different sockets pay a cross-socket transfer penalty that auto-detection also does not optimize for; it uses any visible GPU regardless of topology.
...
Use the same local TP size on every node:
...
The ring backend is a Linux-only distributed transport selected by `RING_CONFIG`. Use it only when you intentionally want the `ring` transport; for CUDA tensor parallelism across machines, prefer multi-node NCCL.

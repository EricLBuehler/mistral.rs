...
`required` is enforced after generation. If the model produces normal text instead of a valid tool call, the request errors rather than returning that text.

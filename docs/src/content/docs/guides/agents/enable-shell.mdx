...
Chat Completions streaming emits the same work as mistral.rs `agentic_tool_call_progress` SSE events. The [agentic runtime guide](/guides/agents/agentic-runtime/) covers the event stream and files contract in depth.
...
For deployments where shell commands must leave the mistralrs host entirely, use [`--tool-dispatch-url`](/guides/agents/tool-calling-basics/#dispatch-url) and expose a custom executor instead of enabling built-in shell.

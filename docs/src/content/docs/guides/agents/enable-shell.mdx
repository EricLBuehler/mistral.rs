...
The shell tool can surface files by naming them in its `outputs` argument. Entries are paths relative to the shell working directory:
...
Files named in request `files` are merged into shell `outputs` automatically. Files written but not listed in either place remain internal to the shell session.

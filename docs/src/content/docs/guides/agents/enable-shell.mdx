...
When sandbox CPU limits are enforced and the configured shell timeout exceeds `max_cpu_secs`, mistral.rs raises `max_cpu_secs` to match and logs a warning. This keeps long-running Skills from being capped by a lower CPU limit.

...
The loop's fallback cap is 256 tool rounds unless `--max-tool-rounds` says otherwise. On Linux and macOS, code and shell execution are [sandboxed by default](/mistral.rs/reference/sandbox/) (`--sandbox auto`). The default timeouts are 60 seconds for Python code execution and 600 seconds for shell execution.

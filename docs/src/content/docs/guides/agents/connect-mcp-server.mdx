...
`max_concurrent_calls` caps in-flight MCP calls (default 10). `tool_timeout_secs` is the per-call timeout (default 30s). `Http` and `WebSocket` sources can override the timeout per-source.

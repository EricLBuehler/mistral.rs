...
`--agent` enables the built-in tool runtime, including the executor used by OpenAI-compatible Skills. For executor flags, sandboxing, and approvals, see [shell execution](/mistral.rs/guides/agents/enable-shell/) and [permissions and approvals](/mistral.rs/guides/agents/permissions-and-approvals/).
...
- `input_file` materialization for Responses file attachments.
...
- OpenAI's general `POST /v1/files` upload API.

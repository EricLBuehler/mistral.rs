...
A single engine thread is the unit of concurrency. Concurrent requests to the same model share a queue and are batched by the scheduler. Requests to different models go to different threads and run independently. Multi-model routing and rehydration of unloaded models are covered in [running multiple models](/mistral.rs/guides/serve/multiple-models/).
...
Server-side tool calling runs in the outer engine loop, inside one HTTP request: inference until a tool call, tool execution, history append, resume, until a non-tool-call response or the round cap. Loop semantics, entry conditions, and configuration live in [agentic runtime for apps](/mistral.rs/guides/agents/agentic-runtime/).

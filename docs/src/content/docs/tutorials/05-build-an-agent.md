...
Search and code execution are off by default. Each carries a cost: network access for search, a Python subprocess for code execution. On Linux and macOS, code execution is sandboxed by default with `--sandbox auto`.

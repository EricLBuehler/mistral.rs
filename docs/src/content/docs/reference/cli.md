...
| `--code-exec-permission <mode>` | `auto` | `auto`, `ask`, or `deny`. `ask` prompts before Python execution in `mistralrs run` and `mistralrs serve`; `deny` blocks execution while still surfacing the tool call. |

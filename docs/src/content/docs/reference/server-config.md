...
| `--code-exec-permission` | `runtime.code_exec_permission` | `auto` | `auto`, `ask`, or `deny`. `ask` prompts in the server terminal before Python execution. Requests may tighten this policy but not loosen it. |

...
| `--code-exec-permission` | `runtime.code_exec_permission` | `auto` | `auto`, `ask`, or `deny`. HTTP `ask` approvals are delivered over streaming SSE and resolved with `POST /v1/agent/approvals/{approval_id}`. Requests may tighten this policy but not loosen it. |

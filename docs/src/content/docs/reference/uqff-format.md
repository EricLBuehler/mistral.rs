...
- Token embeddings and output heads use the higher-precision default documented in [quantization types](/mistral.rs/reference/quantization-types/). For example, an `afq4` shard set stores those tensors as AFQ6, while a `q4k` shard set stores them as Q6K.

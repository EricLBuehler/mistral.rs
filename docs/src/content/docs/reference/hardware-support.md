...
| CUDA 13.1+ on Blackwell / `sm100`, `sm120`, `sm121` | `cuda131` | CUDA fallback kernels; no cuTile |
...
Each artifact bundles the CUDA runtime libraries it needs, so its native CUDA paths do not require a toolkit at runtime. The installed NVIDIA driver still has to be new enough for that artifact's toolkit lane. cuTile-capable archives do not redistribute NVIDIA's `tileiras` developer tool; install it separately by following the [MoE backend setup](/mistral.rs/developer/moe-backends/), or the binary uses its native CUDA routed-LoRA and CUTLASS MoE fallbacks.
...
| cuTile MoE backend | Ampere/Ada (8.x) and Blackwell+ (10.x/12.x) with CUDA >= 13.2; Hopper (9.0) with CUDA >= 13.3 |

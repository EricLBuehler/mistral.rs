...
| `MISTRALRS_CUTILE_TUNE` | Controls the cuTile kernel autotuner that runs during warmup on cuTile builds. `auto` (default) times candidate tile configs for each MoE expert shape on the first load and reuses the persisted winners afterwards, `force` re-measures, and `off` uses the built-in policies. |
| `MISTRALRS_CUTILE_TUNE_CACHE` | Path of the autotuner's JSON cache. Defaults to `cutile_tune.json` under `XDG_CACHE_HOME/mistralrs`, then `~/.cache/mistralrs`. Entries are keyed by GPU, mistral.rs version, kernel, and expert shape. |

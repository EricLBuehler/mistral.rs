...
| `MISTRALRS_ALLOW_RUNTIME_LORA_UPDATING` | Set to `1`, `true`, `yes`, or `on` to enable runtime LoRA load and unload endpoints. Disabled by default; read-only listing is always available. See [LoRA adapters](/mistral.rs/guides/customize/lora-adapters/#enable-http-mutation). |

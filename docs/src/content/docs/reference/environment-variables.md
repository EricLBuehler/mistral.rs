...
| `MISTRALRS_INSTALL_TAG` | Pins the installers to a specific release tag (e.g. `v0.8.20`): the prebuilt is downloaded from that release, and a source build checks out that git tag. Default is the latest stable release (prebuilt) or latest `master` (source). |

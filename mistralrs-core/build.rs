...
fn main() -> Result<()> {
    ...
    for kernel_file in KERNEL_FILES.iter() {
        println!("cargo:rerun-if-changed={PATH}{kernel_file}");
    }
    println!("cargo:rerun-if-changed={PATH}**.h");
    println!("cargo:rerun-if-changed={PATH}**.cuh");
    ...
    let kernel_dir = PathBuf::from(PATH);

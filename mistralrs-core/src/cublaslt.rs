...
impl CublasLTMatmul {
    pub fn fwd_f16(
        ...
        let mut out = if let Some(c) = &self.c {
            ...
            // Allocate out tensor
            unsafe {
                dev.alloc::<f16>(out_shape.elem_count()).unwrap() // todo UNWRAP
            }
        };
    ...
    pub fn fwd_bf16(
        ...
        let mut out = if let Some(c) = &self.c {
            ...
            // Allocate out tensor
            unsafe {
                dev.alloc::<bf16>(out_shape.elem_count()).unwrap() // todo UNWRAP
            }
        };
    ...
    pub fn fwd_f32(
        ...
        let mut out = if let Some(c) = &self.c {
            ...
            // Allocate out tensor
            unsafe {
                dev.alloc::<f32>(out_shape.elem_count()).unwrap() // todo UNWRAP
            }
        };
...
impl CublasLTBatchMatmul {
    ...
    pub fn fwd_f32(
        ...
        let (mut out, stride_c) = if let Some(c) = &self.c {
            ...
            (
                // TODO: unwrap
                unsafe { dev.alloc::<f32>(out_shape.elem_count()).unwrap() },
                (n * m),
            )
        };

...
struct CopyBlocksParams {
  int num_layers;
  int num_pairs;
  int numel_per_block;
  ScalarType dtype;
};
void copy_blocks(void *key_cache_ptrs, void *value_cache_ptrs,
                 const void *block_mapping, CopyBlocksParams params) {
  // Launch the kernel.
  dim3 grid(params.num_layers, params.num_pairs);
  dim3 block(std::min(1024, params.numel_per_block));
  cudaStream_t stream = 0;

  switch (params.dtype) {
  case DATA_F16: {
    vllm::copy_blocks_kernel<__half><<<grid, block, 0, stream>>>(
        reinterpret_cast<int64_t *>(key_cache_ptrs),
        reinterpret_cast<int64_t *>(value_cache_ptrs),
        reinterpret_cast<const int64_t *>(block_mapping),
        params.numel_per_block);
    break;
  }
  case DATA_BF16: {
    vllm::copy_blocks_kernel<__nv_bfloat16><<<grid, block, 0, stream>>>(
        reinterpret_cast<int64_t *>(key_cache_ptrs),
        reinterpret_cast<int64_t *>(value_cache_ptrs),
        reinterpret_cast<const int64_t *>(block_mapping),
        params.numel_per_block);
  ...
  case DATA_F32: {
    vllm::copy_blocks_kernel<float><<<grid, block, 0, stream>>>(
        reinterpret_cast<int64_t *>(key_cache_ptrs),
        reinterpret_cast<int64_t *>(value_cache_ptrs),
        reinterpret_cast<const int64_t *>(block_mapping),
        params.numel_per_block);
...
extern "C" {
void reshape_and_cache(const void *key, const void *value, void *key_cache,
                       void *value_cache, const void *slot_mapping,
                       RehshapeCacheKernelParams params) {
  dim3 grid(params.num_tokens);
  dim3 block(std::min(params.num_heads * params.head_size, 512));
  cudaStream_t stream = 0;
  switch (params.scalar_type) {
  case DATA_F16: {
    vllm::reshape_and_cache_kernel<__half><<<grid, block, 0, stream>>>(
        ...
        reinterpret_cast<const int64_t *>(slot_mapping), params.key_stride,
        params.value_stride, params.num_heads, params.head_size,
        params.block_size, params.x);
    break;
  }
  case DATA_BF16: {
    vllm::reshape_and_cache_kernel<__nv_bfloat16><<<grid, block, 0, stream>>>(
  ...
  case DATA_F32: {
    vllm::reshape_and_cache_kernel<float><<<grid, block, 0, stream>>>(
...
//     torch::Tensor
//         &key_cache, // [in]  [num_blocks, num_heads, head_size/x, block_size,
//         x]
//     torch::Tensor
...
//         vllm::gather_cached_kv_kernel_optimized<scalar_t>
//             <<<grid, block, 0, stream>>>(
//                 key.data_ptr<scalar_t>(), value.data_ptr<scalar_t>(),
//                 key_cache.data_ptr<scalar_t>(),
//                 value_cache.data_ptr<scalar_t>(),
//                 slot_mapping.data_ptr<int>(), key_stride, value_stride,
//                 num_heads, head_size, block_size, x);
//       });
// }
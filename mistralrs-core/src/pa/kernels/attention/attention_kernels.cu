...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                   \
  VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                        \
      ((void *)vllm::paged_attention_v1_kernel<T, HEAD_SIZE, BLOCK_SIZE,       \
                                               NUM_THREADS>),                  \
      shared_mem_size);                                                        \
...
//   vllm.worker.worker._check_if_can_support_max_seq_len
...
template <typename T, int BLOCK_SIZE, int NUM_THREADS = 128>
void paged_attention_v1_launcher(void *out, const void *query,
                                 const void *key_cache, const void *value_cache,
                                 const void *block_tables,
                                 const void *context_lens,
                                 const void *alibi_slopes,
                                 PagedAttentionV1Params params) {
  int num_kv_heads = params.num_kv_heads;
  int num_seqs = params.num_seqs;
  int num_heads = params.num_heads;
  int head_size = params.head_size;
  int max_num_blocks_per_seq = params.max_num_blocks_per_seq;
  int q_stride = params.q_stride;
  int kv_block_stride = params.kv_block_stride;
  int kv_head_stride = params.kv_head_stride;
  ...
  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int padded_max_context_len =
      DIVIDE_ROUND_UP(params.max_context_len, BLOCK_SIZE) * BLOCK_SIZE;
  int logits_size = padded_max_context_len * sizeof(float);
  int outputs_size = (NUM_WARPS / 2) * head_size * sizeof(float);
  // Python-side check in
  // vllm.worker.worker._check_if_can_support_max_seq_len
...
void paged_attention_v1(
    ...
    const void *key_cache,    // [num_blocks, num_kv_heads,
                              // head_size/x, block_size, x]
    const void *value_cache,  // [num_blocks, num_kv_heads,
                              // head_size, block_size]
...
template <typename T, int BLOCK_SIZE, int NUM_THREADS = 128,
          int PARTITION_SIZE = 512>
void paged_attention_v2_launcher(void *out, void *exp_sums, void *max_logits,
                                 void *tmp_out, void *query, void *key_cache,
                                 void *value_cache, void *block_tables,
                                 void *context_lens, const void *alibi_slopes,
                                 PagedAttentionV2Params params) {
  int num_seqs = params.num_seqs;
  int num_heads = params.num_heads;
  int head_size = params.head_size;
  int max_num_blocks_per_seq = params.max_num_blocks_per_seq;
  ...
  T *key_cache_ptr = reinterpret_cast<T *>(key_cache);
  T *value_cache_ptr = reinterpret_cast<T *>(value_cache);
  int *block_tables_ptr = reinterpret_cast<int *>(block_tables);
  int *context_lens_ptr = reinterpret_cast<int *>(context_lens);

  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int max_num_partitions =
      DIVIDE_ROUND_UP(params.max_context_len, PARTITION_SIZE);
  int logits_size = PARTITION_SIZE * sizeof(float);
  int outputs_size = (NUM_WARPS / 2) * head_size * sizeof(float);
...
#define CALL_V2_LAUNCHER(T, BLOCK_SIZE)                                        \
  paged_attention_v2_launcher<T, BLOCK_SIZE>(                                  \
      ...
      block_tables, context_lens, alibi_slopes, params);
...
void paged_attention_v2(void *out, void *exp_sums, void *max_logits,
                        void *tmp_out, void *query, void *key_cache,
                        void *value_cache, void *block_tables,
                        void *context_lens, const void *alibi_slopes,
                        PagedAttentionV2Params params) {

//! Parser for Gemma 4 channel-based reasoning output.
//!
//! Gemma 4 uses `<|channel>thought\n...<channel|>` to wrap chain-of-thought
//! reasoning content. This module provides incremental parsing to extract
//! reasoning content separately from final response content.
//!
//! Similar to the Harmony format parser but for Gemma 4's channel system.

const CHANNEL_OPEN_TAG: &str = "<|channel>thought\n";
const CHANNEL_CLOSE_TAG: &str = "<channel|>";

/// Context for tracking Gemma 4 channel-based reasoning parsing state within a sequence.
///
/// This provides incremental parsing of token streams containing channel tags,
/// with delta extraction for streaming responses.
pub struct GemmaChannelContext {
    /// Buffer for handling partial tags split across token boundaries
    buffer: String,
    /// Whether we're currently inside a channel block
    in_channel: bool,
    /// Accumulated reasoning content (inside channel blocks)
    accumulated_reasoning: String,
    /// Accumulated final content (outside channel blocks)
...
impl GemmaChannelContext {
    /// Create a new GemmaChannelContext.
    pub fn new() -> Self {
        Self {
            buffer: String::new(),
            in_channel: false,
            ...
            if self.in_channel {
                ...
                if let Some(end_pos) = self.buffer.find(CHANNEL_CLOSE_TAG) {
                    ...
                    if partial_len > 0 {
                        ...
                        // No partial tag, process all content as reasoning
                ...
                if let Some(start_pos) = self.buffer.find(CHANNEL_OPEN_TAG) {
                    ...
                    // Remove processed part including the tag
                    self.buffer =
                        self.buffer[start_pos + CHANNEL_OPEN_TAG.len()..].to_string();
                } else {
    ...
    ///
    /// If there's an unclosed channel block, treat any remaining buffer
    ...
    pub fn finalize(&mut self) {
        ...
        if !self.buffer.is_empty() {
            if self.in_channel {
                // Unclosed channel block - remaining buffer is reasoning
                self.accumulated_reasoning.push_str(&self.buffer);
            } else {
                // Not in channel block - remaining buffer is content
                self.accumulated_content.push_str(&self.buffer);
            }
            self.buffer.clear();
        }
    ...
    /// Get the accumulated reasoning content (inside channel blocks).
    ///
    ...
    /// Get the accumulated content (outside channel blocks).
    ///
    ...
    /// Check if currently inside a channel block.
    pub fn is_in_channel(&self) -> bool {
        self.in_channel
    }
}

impl Default for GemmaChannelContext {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_simple_channel_block() {
        let mut ctx = GemmaChannelContext::new();

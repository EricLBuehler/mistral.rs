...
impl FromStr for NormalLoaderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            a => Err(format!("Unknown architecture `{a}`. Possible architectures: `mistral`, `gemma`, `mixtral`, `llama`, `phi2`, `phi3`, `qwen2`, `gemma2`, `starcoder2`, `phi3.5moe`, `deepseekv2`, `deepseekv3`, `qwen3`, `glm4`, `glm4moelite`, `glm4moe`, `qwen3moe`, `smollm3`, `granitemoehybrid`, `gpt_oss`, `qwen3next`, `qwen3_5`, `qwen3_5moe`.")),
        }
    }
}
...
impl Display for NormalLoaderType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Qwen3_5MoE => write!(f, "qwen3_5moe"),
        }
    }
}
...
impl AutoNormalLoader {
    fn get_loader(config: &str) -> Result<Box<dyn NormalModelLoader>> {
        ...
        match tp {
            ...
            NormalLoaderType::Qwen3_5 => Ok(Box::new(Qwen3_5Loader)),
            NormalLoaderType::Qwen3_5MoE => Ok(Box::new(Qwen3_5MoELoader)),
        }
    }
}
...
// ======================== Qwen3.5 (dense) loader

/// [`NormalLoader`] for a Qwen3.5 (dense) model.
///
/// [`NormalLoader`]: https://docs.rs/mistralrs/latest/mistralrs/struct.NormalLoader.html
pub struct Qwen3_5Loader;

impl NormalModelLoader for Qwen3_5Loader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        let cfg: crate::models::qwen3_5::Config = serde_json::from_str(config)?;

        Ok(Box::new(models::qwen3_5::Model::new(
            &cfg,
            vb,
            self.is_gptx(config)?,
            normal_loading_metadata,
...
impl DeviceMappedModelLoader for Qwen3_5Loader {
    ...
    fn layer_sizes_in_bytes(
        ...
        let per_layer_elems = {
            ...
            let mlp = 3 * cfg.hidden_size * cfg.intermediate_size / weight_pack_factor;

            input_layernorm + post_attention_layernorm + q_proj + k_proj + v_proj + o_proj + mlp
        };
        Ok(vec![
            per_layer_elems * dtype.size_in_bytes();
            cfg.num_hidden_layers
...
/// [`NormalLoader`] for a Qwen3.5 MoE model.
///
/// [`NormalLoader`]: https://docs.rs/mistralrs/latest/mistralrs/struct.NormalLoader.html
pub struct Qwen3_5MoELoader;

impl NormalModelLoader for Qwen3_5MoELoader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        let cfg: crate::models::qwen3_5_moe::Config = serde_json::from_str(config)?;

        Ok(Box::new(models::qwen3_5_moe::Model::new(
            &cfg,
            vb,
            self.is_gptx(config)?,
            normal_loading_metadata,
    ...
    fn load_xlora(
        ...
        anyhow::bail!("Qwen3.5-MoE does not support X-LoRA")
    }
    fn is_gptx(&self, _: &str) -> Result<bool> {
        Ok(true)
    }

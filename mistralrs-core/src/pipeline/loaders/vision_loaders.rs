...
use crate::vision_models::qwen3_5_vl::{
    Config as Qwen3_5VLConfig, Qwen3_5VLModel, Qwen3_5VLProcessor,
};
...
impl FromStr for VisionLoaderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "qwen3_5_vl" => Ok(Self::Qwen3_5VL),
            "voxtral" => Ok(Self::Voxtral),
            a => Err(format!("Unknown architecture `{a}`. Possible architectures: `phi3v`, `idefics2`, `llava_next`, `llava`, `vllama`, `qwen2vl`, `idefics3`, `minicpmo`, `phi4mm`, `qwen2_5vl`, `gemma3`, `mistral3`, `llama4`, `gemma3n`, `qwen3vl`, `qwen3vlmoe`, `qwen3_5_vl`, `voxtral`.")),
        }
    }
}
...
impl std::fmt::Display for VisionLoaderType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let name = match self {
            ...
            VisionLoaderType::Qwen3_5VL => "qwen3_5_vl",
...
impl AutoVisionLoader {
    fn get_loader(config: &str) -> Result<Box<dyn VisionModelLoader>> {
        ...
        Ok(match tp {
            ...
            VisionLoaderType::Qwen3_5VL => Box::new(Qwen3_5VLLoader),
...
/// [`VisionLoader`] for a Qwen3.5 MoE model.
///
/// [`VisionLoader`]: https://docs.rs/mistralrs/latest/mistralrs/struct.VisionLoader.html
pub struct Qwen3_5VLLoader;

pub struct Qwen3_5VLPrefixer;

impl MultimodalPromptPrefixer for Qwen3_5VLPrefixer {
    // No-op: With MessagesAction::Keep, the chat template handles image tokens
    // when it sees {"type": "image"} entries in the content.
}

impl VisionModelLoader for Qwen3_5VLLoader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        let cfg: Qwen3_5VLConfig = serde_json::from_str(config)?;
        Ok(Box::new(Qwen3_5VLModel::new(
            &cfg,
            vb,
            self.is_gptx(config),
            normal_loading_metadata,
    ...
    fn get_config_repr(&self, config: &str) -> Result<Box<dyn Debug>> {
        let config: Qwen3_5VLConfig = serde_json::from_str(config)?;
        Ok(Box::new(config))
    }
    fn get_processor(
        &self,
        ...
        Arc::new(Qwen3_5VLProcessor::new(max_edge))
    }
    fn supports_paged_attention(&self, _config: &str) -> bool {
        true
    }
...
impl IsqModelLoader for Qwen3_5VLLoader {
    fn isq_layer_regexes(&self, _config: &str) -> Result<Vec<Regex>> {
        Ok(vec![
            Regex::new(r"lm_head\.(weight|bias)$")?,
            ...
            // MoE experts
            Regex::new(
                r"model\.language_model\.layers\.(\d+)\.mlp\.experts\.(\d+)\.(gate_proj|up_proj|down_proj)\.(weight|bias)$",
            )?,
        ])
    }
    fn immediate_isq_predicates(&self, config: &str) -> Result<Vec<Regex>> {
...
impl DeviceMappedModelLoader for Qwen3_5VLLoader {
    fn mapped_max_act_size_elems(
        &self,
        config: &str,
        params: &AutoDeviceMapParams,
    ...
    fn layer_sizes_in_bytes(
        ...
        for layer_idx in 0..text_cfg.num_hidden_layers {
            ...
            // Attention weights (approximate for both full and linear attention layers)
            let size_in = text_cfg.hidden_size;
            let size_q = text_cfg.head_dim * text_cfg.num_attention_heads;
            let size_kv = text_cfg.head_dim * text_cfg.num_key_value_heads;
            let q_proj = size_in * size_q / weight_pack_factor;

 ...
 *
 * @param input             [M, K] - Input activations for all tokens.
 * @param weights           [num_experts, K, N] - Expert weight matrices (transposed layout).
 * @param sorted_token_ids  [M] - Indices of tokens sorted by expert assignment.
 * @param expert_ids        [M] - Expert ID for each token.
 * @param topk_weights      [M] (optional) - Per-token gating weights (nullptr if not used).
 * @param output            [M, N] - Output activations for all tokens.
    ...
    const int M, const int N, const int K)
{
    ...
    // s_weights: Caches the [N, K] weight matrix tile (conceptually transposed from global)
    // Layout: [BLOCK_N_TILE][BLOCK_K_TILE] for coalesced compute
    __shared__ T s_weights[BLOCK_N_TILE][BLOCK_K_TILE];
    ...
    for (int k_start_vec = 0; k_start_vec < k_vec_dim_size; k_start_vec += k_vec_tile_size) {
        ...
        // We need to load weight[k_start:k_start+BLOCK_K_TILE, n] for each n in our tile
        // Each thread tid_n loads weights for column n, iterating over k
        for (int k_inner = tid_k; k_inner < BLOCK_K_TILE; k_inner += BLOCK_K_THREADS) {
...
// Transposed weight variant: weights are [num_experts, size_k, size_n] instead of [num_experts, size_n, size_k]
extern "C" void moe_gemm_transposed(
    ...
    if (dtype == 0) {
        moe_gemm_transposed_kernel<half, half2, BLOCK_N_TILE, BLOCK_K_TILE, BLOCK_K_THREADS>
            <<<grids, blocks, 0, stream>>>(
            reinterpret_cast<const half*>(input), reinterpret_cast<const half*>(weights),
            sorted_token_ids, expert_ids,
            topk_weights, reinterpret_cast<half*>(output),
        ...
        moe_gemm_transposed_kernel<nv_bfloat16, nv_bfloat162, BLOCK_N_TILE, BLOCK_K_TILE, BLOCK_K_THREADS>
            <<<grids, blocks, 0, stream>>>(
            reinterpret_cast<const nv_bfloat16*>(input), reinterpret_cast<const nv_bfloat16*>(weights),
            sorted_token_ids, expert_ids,
            topk_weights, reinterpret_cast<nv_bfloat16*>(output),

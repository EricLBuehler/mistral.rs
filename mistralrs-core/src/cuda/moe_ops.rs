...
impl FusedMoeForward {
    ...
    pub fn forward_optimized(
        ...
        if hd_d != hidden_dim {
            candle_core::bail!("Hidden dimension mismatch for down weights");
        }
        if id_g != id_u || id_u != id_d {
        ...
        match device {
            Device::Cuda(cuda_device) => {
                // Use chunked execution for large batches
                let chunk_size = 8192; // Process 8K tokens at a time
                if num_tokens > chunk_size {
                    self.cuda_fwd_chunked(
                        input,
                        gate_weights,
                        up_weights,
                        down_weights,
        ...
        match (input.dtype(), expert_indices.dtype()) {
            (DType::F32, DType::U32) => {
                ...
                unsafe {
                    ...
                    ffi::fused_moe_forward_optimized_f32(
                        input_ptr as *const c_void,
                        gate_ptr as *const c_void,
                        up_ptr as *const c_void,
                        down_ptr as *const c_void,
            ...
            (DType::F16, DType::U32) => {
                ...
                unsafe {
                    ...
                    // For now, use the regular forward for F16
                    ffi::fused_moe_forward_f16(
                        input_ptr as *const c_void,
                        gate_ptr as *const c_void,
                        up_ptr as *const c_void,
            ...
            (DType::BF16, DType::U32) => {
                ...
                unsafe {
                    ...
                    // For now, use the regular forward for BF16
                    ffi::fused_moe_forward_bf16(
                        input_ptr as *const c_void,
                        gate_ptr as *const c_void,
                        up_ptr as *const c_void,
            ...
            _ => candle_core::bail!("Unsupported dtype combination for FusedMoeForward: {:?} with {:?} indices", input.dtype(), expert_indices.dtype()),
        }
    ...
    fn cuda_fwd_chunked(
        ...
        // Get storage and layouts (same as optimized version)
        let (input_storage, input_layout) = input.storage_and_layout();
        let (gate_storage, gate_layout) = gate_weights.storage_and_layout();
        let (up_storage, up_layout) = up_weights.storage_and_layout();
        let (down_storage, down_layout) = down_weights.storage_and_layout();
        ...
        match (input.dtype(), expert_indices.dtype()) {
            (DType::F32, DType::U32) => {
                ...
                unsafe {
                    ...
                    ffi::fused_moe_forward_chunked_f32(
                        input_ptr as *const c_void,
                        gate_ptr as *const c_void,
                        up_ptr as *const c_void,
                        down_ptr as *const c_void,
            ...
            (DType::F16, DType::U32) => {
                // For F16, fall back to regular forward for now
                return self.cuda_fwd(
                    input,
                    gate_weights,
                    up_weights,
                    down_weights,
            ...
            (DType::BF16, DType::U32) => {
                // For BF16, fall back to regular forward for now
                return self.cuda_fwd(
                    input,
                    gate_weights,
                    up_weights,
            ...
            _ => candle_core::bail!("Unsupported dtype combination for chunked kernel: {:?} with {:?} indices", input.dtype(), expert_indices.dtype()),
        }

...
struct Attention {
    ...
    #[cfg(all(feature = "cuda", target_family = "unix"))]
    mla_weights: Option<Mutex<Option<(Tensor, Tensor)>>>,
}
...
impl Attention {
    ...
    fn new(
        ...
        #[cfg(all(feature = "cuda", target_family = "unix"))]
        let mla_weights = if paged_attn.is_some() {
            if matches!(
    ...
    #[cfg(all(feature = "cuda", target_family = "unix"))]
    fn compute_mla_weights(&self, device: &Device) -> Result<(Tensor, Tensor)> {
        let mut w = self.kv_b_proj.dequantize_w()?;
        if !w.device().same_device(device) {
            w = w.to_device(device)?;
        }
        let (out_dim, in_dim) = w.dims2()?;
        if in_dim != self.cfg.kv_lora_rank {
            candle_core::bail!(
        ...
        let per_head_dim = self.cfg.qk_nope_head_dim + self.cfg.v_head_dim;
        if out_dim != self.num_attention_heads * per_head_dim {
            candle_core::bail!(
                "kv_b_proj weight out_dim mismatch: expected {}, got {}",
                self.num_attention_heads * per_head_dim,
                out_dim
            );
        }
        let w = w.reshape((
        ...
        let w_uk = w
            .narrow(D::Minus2, 0, self.cfg.qk_nope_head_dim)?
            .contiguous()?;
        let w_uv = w
            .narrow(D::Minus2, self.cfg.qk_nope_head_dim, self.cfg.v_head_dim)?
            .contiguous()?;
    ...
    #[cfg(all(feature = "cuda", target_family = "unix"))]
    fn get_mla_weights(&self, device: &Device) -> Result<(Tensor, Tensor)> {
        let Some(mla_weights) = &self.mla_weights else {
        ...
        let (w_uk, w_uv_t) = self.compute_mla_weights(device)?;
        *guard = Some((w_uk.clone(), w_uv_t.clone()));
        Ok((w_uk, w_uv_t))
    }

    #[cfg(all(feature = "cuda", target_family = "unix"))]
    fn mla_decode_forward(
        &self,
        q_nope: &Tensor,
        q_pe: &Tensor,
        ckv: &Tensor,
        ...
        let ckv_flat = ckv
            .contiguous()?
            .reshape((bs * seq_len, self.cfg.kv_lora_rank))?;
        let k_pe_flat = k_pe
            .squeeze(1)?
            .contiguous()?
            .reshape((bs * seq_len, self.cfg.qk_rope_head_dim))?;
    ...
    fn mla_cache_forward(
        ...
        if !needs_prefix && attention_mask.is_some() {
            ...
            if total_prefix_tokens > 0 {
                ...
                let cu_seq_lens = Tensor::from_vec(cu_seq_lens, (bs + 1,), &Device::Cpu)?
                    .to_device(q.device())?;
                let token_to_seq =
                ...
                let mut kv_prefix = self.kv_b_proj.forward_autocast(&ckv_prefix)?;
                kv_prefix = kv_prefix.reshape((
                    total_prefix_tokens,
                    self.num_attention_heads,
                    self.cfg.qk_nope_head_dim + self.cfg.v_head_dim,
                ))?;
                let kv_prefix_split = kv_prefix
                    .split(&[self.cfg.qk_nope_head_dim, self.cfg.v_head_dim], D::Minus1)?;
            ...
            for (seq_idx, cur_len) in cur_lens.iter().enumerate() {
                ...
                if cur_len == 0 {
                    outputs.push(Tensor::zeros(
                        (seq_len, self.num_attention_heads, self.cfg.v_head_dim),
                        q.dtype(),
                        q.device(),
                    )?);
                    continue;
    ...
    fn forward(
        ...
        let use_mla_decode = {
            #[cfg(all(feature = "cuda", target_family = "unix"))]
            {
                attention_mask.is_none()
                    ...
                    && matches!(q_nope.device(), Device::Cuda(_))
                    && metadata
                        .as_ref()
        ...
        let mut attn_out = if use_mla_decode {
            self.mla_decode_forward(&q_nope, &q_pe, &ckv, &k_pe, &metadata, bs, seq_len)?
        } else {
            ...
            let use_mla_cache = {
                #[cfg(all(feature = "cuda", target_family = "unix"))]
                {
                    self.paged_attn.is_some() && matches!(q.device(), Device::Cuda(_))
                }
                #[cfg(not(all(feature = "cuda", target_family = "unix")))]
                {
                    false

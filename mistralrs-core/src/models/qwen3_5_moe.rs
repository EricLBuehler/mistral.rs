...
//! Qwen3.5 MoE model (hybrid attention: FullAttention + GatedDeltaNet, MoE feed-forward).
...
// ====================== Full Attention (same gated-query pattern) ======================
...
impl Model {
    pub fn new(
        ...
        for i in NiceProgressBar::<_, 'b'>(
            ...
            // Some layers use dense MLP (mlp_only_layers config)
            let ffn = if cfg.mlp_only_layers.contains(&i) {

...
use crate::{
    ...
    get_mut_arcmutex,
    layers::{CausalMasker, MatMul, PhiRotaryEmbedding, Sdpa},
...
impl Mlp {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let mut res = MatMul.qmethod_matmul(&current_hidden_states, &**w2)?;
    ...
    fn to_device(&self, device: &Device) -> Result<()> {
        let mut w1 = get_mut_arcmutex!(self.w1);
        let mut w2 = get_mut_arcmutex!(self.w2);
        let mut w3 = get_mut_arcmutex!(self.w3);
        *w1 = w1.clone().to_device(device)?;
        *w2 = w2.clone().to_device(device)?;
        *w3 = w3.clone().to_device(device)?;
        Ok(())
    }
}

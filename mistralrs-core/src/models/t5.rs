...
use crate::{
    ...
    pipeline::{Cache, IsqModel, Seq2SeqModel},
};
...
impl IsqModel for T5ForConditionalGeneration {
    fn get_tensors(&mut self) -> (Vec<(&mut QMatMul, Option<usize>)>, &dyn DeviceMapper) {
        todo!()
    }
}

impl Seq2SeqModel for T5ForConditionalGeneration {
    fn forward_decode(
        &mut self,
        input_ids: &Tensor,
        encoder_hidden_states: &Tensor,
    ) -> candle_core::Result<Tensor> {
        self.decode(input_ids, encoder_hidden_states)
    }
    fn forward_encode(&mut self, prompt_input_ids: &Tensor) -> candle_core::Result<Tensor> {
        self.encode(prompt_input_ids)
    }
    fn device(&self) -> &Device {
        &self.device
    }
    fn max_seq_len(&self) -> usize {
        512 // Is this really true for all T5 models?
    }
    fn cache(&self) -> &Cache {
        todo!()
    }

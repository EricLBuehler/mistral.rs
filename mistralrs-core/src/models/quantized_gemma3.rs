...
impl LayerWeights {
    fn forward_attn(
        ...
        let (q, k) = if self.use_sliding_window {
            self.rotary_local.forward(&q, &k, &positions)?
        } else {
            self.rotary_global.forward(&q, &k, &positions)?
        };
...
impl ModelWeights {
    pub fn forward(
        ...
        let x = self.output.forward(&x.contiguous()?)?;

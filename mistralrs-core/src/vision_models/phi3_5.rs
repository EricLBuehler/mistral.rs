...
// https://huggingface.co/microsoft/Phi-3-mini-4k-instruct/blob/main/modeling_phi3_5_5.py
use candle_core::{
        ...
        if select {
            match (hd_transform, image_set_tensor) {
                ...
                (None, Some(Either::Right(image_set_tensor))) => {
                    ...
                    // https://huggingface.co/microsoft/Phi-3.5-vision-instruct/blob/dbcdaaacf52c8e40cf8de6d6ffa6ff6860e5f256/image_embedding_phi3_5_5_v.py#L259
...
pub(crate) struct Phi3_5VisionSpecificArgs {
...
impl VisionModel for Model {
    fn forward(
        ...
        let Phi3_5VisionSpecificArgs { image_sizes } = *model_specific_args
            .downcast()
            .expect("Cannot downcast into `Phi3_5VisionSpecificArgs`");
        self.forward(
            input_ids,
            pixel_values,
            seqlen_offsets,

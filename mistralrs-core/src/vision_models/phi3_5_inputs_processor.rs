...
use super::{
    ...
    phi3_5::Phi3_5VisionSpecificArgs,
...
// Input processor
pub struct Phi3_5InputsProcessor {
    image_tag_splitter: Regex,
}
// Processor
pub struct Phi3_5Processor {
    inputs_processor: Arc<Phi3_5InputsProcessor>,
}

impl ProcessorCreator for Phi3_5Processor {
    fn new_processor(
        _: Option<ProcessorConfig>,
        _: PreProcessorConfig,
    ) -> Arc<dyn Processor + Send + Sync> {
        Arc::new(Self {
            inputs_processor: Arc::new(Phi3_5InputsProcessor {
...
impl Processor for Phi3_5Processor {
...
impl InputsProcessor for Phi3_5InputsProcessor {
    ...
    fn process_inputs(
        ...
        let Some(tokenizer) = tokenizer else {
            return Box::new(std::iter::once(Err(anyhow::Error::msg(
                "Phi3_5InputProcessor requires a specified tokenizer.",
            ))));
        };
                ...
                text_models_inputs_processor::TextInputsProcessor
                    ...
                    .map(|metadata| {
                        ...
                        let inputs: Box<dyn Any> = Box::new(ModelInputs {
                            ...
                            position_ids,
                            pixel_values: None,
                            model_specific_args: Box::new(Phi3_5VisionSpecificArgs {
                                image_sizes: None,
                            }),
                            paged_attn_meta,
                            flash_meta,
...
impl ImagePreProcessor for Phi3_5InputsProcessor {

...
pub(crate) use crate::vision_models::qwen3_vl::Qwen3VLProcessor as Qwen3_5VLProcessor;
...
impl Qwen3_5VLModel {
    pub fn new(
        ...
        let is_moe = cfg
            .architectures
            .as_ref()
            .map(|a| a.iter().any(|s| s.to_lowercase().contains("moe")))
            .unwrap_or(true); // Default to MoE if not explicitly found

        let text = Qwen3_5VLTextModel::new(
            &text_config,
            vb.clone(),
    ...
    pub fn forward(
        ...
        _rope_img_grid_thw: Option<Tensor>,
        _rope_vid_grid_thw: Option<Tensor>,
        seqlens: Vec<usize>,
        ...
        let _ropeidx_attn_mask = Tensor::stack(&ropeidx_attn_mask_bs, 0)?;
...
impl VisionModel for Qwen3_5VLModel {
    fn forward(
        &self,
        input_ids: &Tensor,
        pixel_values: Option<Tensor>,
...
impl IsqModel for Qwen3_5VLModel {
    fn get_layers(
        &mut self,
    ) -> (
...
impl AnyMoeBaseModelMixin for Qwen3_5VLModel {}

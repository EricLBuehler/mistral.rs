        ...
        if image_grid_thw.is_some() || video_grid_thw.is_some() {
                ...
                .enumerate()
            {
                ...
                let alignment_reference = if existing_l1 > 0.0 {
                    ...
                    tracing::debug!(
                        "Qwen2-VL alignment reference is all zeros at batch_idx={i}; falling back to attention-derived reference"
                    );
                ...
                if llm_len != seq_len || mask_len != seq_len {
                    tracing::debug!(
                        "Qwen2-VL position drift detected (batch_idx={i}, llm_len={llm_len}, mask_len={mask_len}, seq_len={seq_len})"
                    );
                }
...
mod tests {
    ...
    fn llm_policy_accepts_equal_and_plus_one_lengths() -> Result<()> {
        ...
        let aligned_equal =
            align_llm_positions_with_policy(equal, &reference, seq_len, seq_len, "Qwen2-VL", 0)?;
    ...
    fn mask_policy_accepts_equal_and_plus_one_lengths() -> Result<()> {
        ...
        let aligned_equal =
            align_positions_mask_with_policy(equal, seq_len, seq_len, "Qwen2-VL", 0)?;
        assert_eq!(aligned_equal.dims2()?, (3, seq_len));

        let plus_one = Tensor::ones((3, seq_len + 1), DType::U8, dev)?;
        let aligned_plus_one =
            align_positions_mask_with_policy(plus_one, seq_len, seq_len, "Qwen2-VL", 0)?;
        assert_eq!(aligned_plus_one.dims2()?, (3, seq_len));
        Ok(())
    }
    ...
    fn mask_policy_rejects_minus_one_and_plus_two_lengths() -> Result<()> {
        ...
        let err = align_positions_mask_with_policy(minus_one, seq_len, seq_len, "Qwen2-VL", 0)
        ...
        let err = align_positions_mask_with_policy(plus_two, seq_len, seq_len, "Qwen2-VL", 0)

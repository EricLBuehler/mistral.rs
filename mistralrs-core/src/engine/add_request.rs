use crate::{
    ...
    MessageContent, RequestMessage, Response, ResponseOk, ToolCallResponse, WebSearchOptions,
};
...
async fn search_request(this: Arc<Engine>, request: NormalRequest) {
    ...
    pub async fn handle_request(self: Arc<Self>, request: Request) {
        match request {
            Request::Normal(request) => {
                ...
                {
                    search_request(self.clone(), request).await;
                } else {

...
use candle_core::{DType, IndexOp, Layout, Module, Result, Shape, Tensor, D};
...
fn pad1d(xs: &Tensor, pad_l: usize, pad_r: usize, mode: PadMode) -> Result<Tensor> {
    match mode {
        ...
        PadMode::Reflect => candle_core::bail!("pad-mode 'reflect' is not supported"),
...
impl candle_core::CustomOp2 for CodebookEncode {
    ...
    fn cpu_fwd(
        &self,
        lhs_storage: &candle_core::CpuStorage,
        lhs_layout: &Layout,
        rhs_storage: &candle_core::CpuStorage,
        rhs_layout: &Layout,
        ...
        if lhs_dim2 != rhs_dim2 {
            candle_core::bail!(
                "CodebookEncode, mismatch on last dim, {lhs_layout:?} {rhs_layout:?}"
            );
        }
        if lhs_dim2 == 0 {
            candle_core::bail!("CodebookEncode, empty last dim {lhs_layout:?}")
        }
        let lhs = match lhs_layout.contiguous_offsets() {
            None => {
                candle_core::bail!("CodebookEncode, lhs has to be contiguous, got {lhs_layout:?}")
            }
            Some((o1, o2)) => {
        ...
        let rhs = match rhs_layout.contiguous_offsets() {
            None => {
                candle_core::bail!("CodebookEncode, rhs has to be contiguous, got {rhs_layout:?}")
            }
            Some((o1, o2)) => {

...
template <int N, bool RegisterSized> struct MultiplyRn<cutlass::Array<float, N, RegisterSized>> {
  CUTLASS_HOST_DEVICE cutlass::Array<float, N, RegisterSized>
  operator()(const cutlass::Array<float, N, RegisterSized> &a, const cutlass::Array<float, N, RegisterSized> &b) const {
...
template <class OutputType, bool WeightFirst = false> struct KernelConfig {
  ...
  static constexpr int kSwizzle = WeightFirst ? kDecodeSchedulerSwizzle : kSchedulerSwizzle;
  using Output = OutputType;
  using TileShape = std::conditional_t<WeightFirst, Shape<_128, _32, _256>, Shape<_128, _128, _128>>;
  ...
  using WeightGlobal =
      std::conditional_t<WeightFirst,
                         fusion::Sm90ColBroadcast<0, TileShape, float, float, Stride<_1, _0, _0>, 1, false>,
                         fusion::Sm90RowBroadcast<0, TileShape, float, float, Stride<_0, _1, _0>, 1, false>>;
  using ActivationGlobal = fusion::Sm90ScalarBroadcast<float>;
  using WeightScaled =
      fusion::Sm90EVT<fusion::Sm90Compute<MultiplyRn, float, float, cutlass::FloatRoundStyle::round_to_nearest>,
                      fusion::Sm90AccFetch, WeightGlobal>;
  using OutputScaled =
      fusion::Sm90EVT<fusion::Sm90Compute<MultiplyRn, Output, float, cutlass::FloatRoundStyle::round_to_nearest>,
                      WeightScaled, ActivationGlobal>;
  using OutputLayout = std::conditional_t<WeightFirst, cutlass::layout::ColumnMajor, cutlass::layout::RowMajor>;
  using EpilogueSchedule = std::conditional_t<WeightFirst, cutlass::epilogue::TmaWarpSpecialized,
                                              cutlass::epilogue::TmaWarpSpecializedCooperative>;
  using Epilogue = typename cutlass::epilogue::collective::CollectiveBuilder<
      cutlass::arch::Sm120, cutlass::arch::OpClassBlockScaledTensorOp, TileShape, ClusterShape,
      cutlass::epilogue::collective::EpilogueTileAuto, float, float, void, OutputLayout, 8, Output, OutputLayout, 8,
      EpilogueSchedule, OutputScaled>::CollectiveOp;
  using Mainloop = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm120, cutlass::arch::OpClassBlockScaledTensorOp, InputPair, cutlass::layout::RowMajor,
      kWeightAlignment, InputPair, cutlass::layout::ColumnMajor, kWeightAlignment, float, TileShape, ClusterShape,
      cutlass::gemm::collective::StageCountAutoCarveout<static_cast<int>(sizeof(typename Epilogue::SharedStorage))>,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  using Scheduler = std::conditional_t<WeightFirst, cutlass::gemm::StaticPersistentScheduler, void>;
  using Implementation = cutlass::gemm::kernel::GemmUniversal<Shape<int, int, int, int>, Mainloop, Epilogue, Scheduler>;
};
...
struct DecodeBf16Kernel : KernelConfig<cutlass::bfloat16_t, true>::Implementation {
...
static_assert(sizeof(DecodeBf16Kernel::Params) == sizeof(DecodeBf16Kernel::Base::Params));
static_assert(alignof(DecodeBf16Kernel::Params) == alignof(DecodeBf16Kernel::Base::Params));
...
static_assert(sizeof(DecodeF16Kernel::Params) == sizeof(DecodeF16Kernel::Base::Params));
static_assert(alignof(DecodeF16Kernel::Params) == alignof(DecodeF16Kernel::Base::Params));
static_assert(!DecodeF16Kernel::TileScheduler::IsDynamicPersistent);

bool valid_dtype(int32_t dtype) { return dtype == MISTRALRS_NVFP4_BF16 || dtype == MISTRALRS_NVFP4_F16; }

bool valid_kernel(int32_t kernel) { return kernel == MISTRALRS_NVFP4_PREFILL || kernel == MISTRALRS_NVFP4_DECODE_DP32; }

bool valid_context(const mistralrs_nvfp4_context *context) {
  return context && context->device >= 0 && context->sm_count > 0 && valid_dtype(context->dtype) &&
         valid_kernel(context->kernel);
}

bool valid_shape(const mistralrs_nvfp4_shape *shape) {
  return shape && shape->m > 0 && shape->n > 0 && shape->k > 0 && shape->n % kWeightAlignment == 0 &&
...
template <class Kernel> typename Kernel::Arguments make_arguments(const mistralrs_nvfp4_launch &launch) {
  ...
  mainloop.ptr_A = static_cast<const Packed *>(Config::kWeightFirst ? launch.w_packed : launch.a_packed);
  mainloop.dA = cutlass::make_cute_packed_stride(typename Kernel::StrideA{}, make_shape(m, shape.k, 1));
  mainloop.ptr_B = static_cast<const Packed *>(Config::kWeightFirst ? launch.a_packed : launch.w_packed);
  mainloop.dB = cutlass::make_cute_packed_stride(typename Kernel::StrideB{}, make_shape(n, shape.k, 1));
  mainloop.ptr_SFA =
      static_cast<const BlockScale *>(Config::kWeightFirst ? launch.w_scale_swizzled : launch.a_scale_swizzled);
  mainloop.ptr_SFB =
      static_cast<const BlockScale *>(Config::kWeightFirst ? launch.a_scale_swizzled : launch.w_scale_swizzled);
  ...
  typename WeightScaled::Arguments weight_scaled{{}, {launch.weight_global, 0.0f, {}}, {}};
  typename OutputScaled::Arguments output_scaled{weight_scaled, {{0.0f}, {launch.activation_global}, {}}, {}};
  auto stride = cutlass::make_cute_packed_stride(typename Kernel::StrideD{}, make_shape(m, n, 1));
  typename Kernel::EpilogueArguments epilogue{output_scaled, nullptr, stride, static_cast<Output *>(launch.output),
                                              stride};
  ...
  return {cutlass::gemm::GemmUniversalMode::kGemm,
          ...
          mainloop,
          epilogue,
          cutlass::KernelHardwareInfo{launch.context.device, launch.context.sm_count},
          scheduler};
}

int scale_sizes(int32_t rows, int32_t k, size_t *cols, size_t *padded_cols, size_t *bytes) {
  ...
  *padded_cols = (*cols + kScaleCols - 1) / kScaleCols * kScaleCols;
  size_t padded_rows = (static_cast<size_t>(rows) + kScaleRows - 1) / kScaleRows * kScaleRows;
...
__host__ __device__ size_t scale_offset(size_t row, size_t col, size_t padded_cols) {
  return (((row / kScaleRows) * (padded_cols / kScaleCols) + col / kScaleCols) * 32 + row % 32) * 16 +
         (row % kScaleRows) / 32 * 4 + col % 4;
}

__global__ void swizzle_scales(const uint8_t *source, uint8_t *dest, size_t rows, size_t cols, size_t padded_cols,
                               size_t bytes) {
  size_t step = static_cast<size_t>(gridDim.x) * blockDim.x;
  for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < bytes; index += step) {
    size_t part = index / 4;
...
int prepare_kernel(const mistralrs_nvfp4_context &context, const cudaDeviceProp &props,
                   mistralrs_nvfp4_resources *resources) {
  if constexpr (Kernel::SharedStorageSize >= kDefaultSharedBytes) {
    auto error = cudaFuncSetAttribute(cutlass::device_kernel<Kernel>, cudaFuncAttributeMaxDynamicSharedMemorySize,
...
template <class Kernel>
int workspace_size(const mistralrs_nvfp4_context &context, const mistralrs_nvfp4_shape &shape, size_t *bytes) {
...
extern "C" int mistralrs_nvfp4_prepare(int32_t device, int32_t dtype, int32_t kernel,
                                       mistralrs_nvfp4_resources *resources) {
  ...
  mistralrs_nvfp4_context context{device, props.multiProcessorCount, dtype, kernel};
  if (kernel == MISTRALRS_NVFP4_DECODE_DP32)
    return dtype == MISTRALRS_NVFP4_BF16 ? prepare_kernel<DecodeBf16Kernel>(context, props, resources)
                                         : prepare_kernel<DecodeF16Kernel>(context, props, resources);
  return dtype == MISTRALRS_NVFP4_BF16 ? prepare_kernel<Bf16Kernel>(context, props, resources)
                                       : prepare_kernel<F16Kernel>(context, props, resources);
}

extern "C" int mistralrs_nvfp4_workspace_size(const mistralrs_nvfp4_context *context,
  ...
  if (context->kernel == MISTRALRS_NVFP4_DECODE_DP32)
    return context->dtype == MISTRALRS_NVFP4_BF16 ? workspace_size<DecodeBf16Kernel>(*context, *shape, bytes)
                                                  : workspace_size<DecodeF16Kernel>(*context, *shape, bytes);
  return context->dtype == MISTRALRS_NVFP4_BF16 ? workspace_size<Bf16Kernel>(*context, *shape, bytes)
                                                : workspace_size<F16Kernel>(*context, *shape, bytes);
}

extern "C" int mistralrs_nvfp4_gemm(const mistralrs_nvfp4_launch *launch) {
  if (!launch || !valid_context(&launch->context) || !valid_shape(&launch->shape) || !aligned(launch->a_packed) ||
      !aligned(launch->w_packed) || !aligned(launch->a_scale_swizzled) || !aligned(launch->w_scale_swizzled) ||
      !aligned(launch->output) || !aligned(launch->weight_global, sizeof(float)) ||
      !aligned(launch->activation_global, sizeof(float)))
    return -static_cast<int>(cudaErrorInvalidValue);
  if (launch->context.kernel == MISTRALRS_NVFP4_DECODE_DP32)
    return launch->context.dtype == MISTRALRS_NVFP4_BF16 ? gemm<DecodeBf16Kernel>(*launch)
                                                         : gemm<DecodeF16Kernel>(*launch);
  return launch->context.dtype == MISTRALRS_NVFP4_BF16 ? gemm<Bf16Kernel>(*launch) : gemm<F16Kernel>(*launch);
}

extern "C" int mistralrs_nvfp4_scale_bytes(int32_t rows, int32_t k, size_t *bytes) {
  size_t cols = 0, padded_cols = 0;
  return scale_sizes(rows, k, &cols, &padded_cols, bytes);
}

extern "C" int mistralrs_nvfp4_swizzle_host(const void *source, void *dest, int32_t rows, int32_t k,
                                            size_t dest_bytes) {
...
extern "C" int mistralrs_nvfp4_swizzle_cuda(const void *source, void *dest, int32_t rows, int32_t k, size_t dest_bytes,
                                            void *stream) {
  size_t cols = 0, padded_cols = 0, bytes = 0;
  int status = scale_sizes(rows, k, &cols, &padded_cols, &bytes);
  if (status)
  ...
  swizzle_scales<<<static_cast<unsigned>(blocks), kSwizzleThreads, 0, static_cast<cudaStream_t>(stream)>>>(
      static_cast<const uint8_t *>(source), static_cast<uint8_t *>(dest), rows, cols, padded_cols, bytes);

...
__global__ void fp8_matmul_tiled(
    ...
    for (int k_tile = 0; k_tile < K; k_tile += BLOCK_K) {
        ...
        for (int i = tid; i < BLOCK_N * BLOCK_K; i += num_threads) {
            ...
            if (gn < N && gk < K) {
                ...
                float s = get_scale(weight_scale, gn, gk, scale_row_stride, block_size_y, block_size_x);
...
__global__ void fp8_moe_gemm(
    ...
    // Pointers
    const __nv_fp8_e4m3* w_row = weights + (size_t)expert_idx * N * K + (size_t)n_idx * K;
...
extern "C" void launch_fp8_indexed_moe_gemm_bf16(
    ...
    fp8_gemm::fp8_moe_gemm<__nv_bfloat16>
        <<<grid, block, 0, stream>>>(
        input, weights, weight_scales, indices, output,
        num_tokens, topk, num_experts, N, K,
        scale_row_stride, block_size_y, block_size_x, input_has_topk_dim

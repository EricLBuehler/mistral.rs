...
constexpr float kMinScale = 1.0f / (kFp8Max * 512.0f);
...
#pragma unroll
    ...
    const float scale = fmaxf(maximum / kFp8Max, kMinScale);
    if (lane == 0) {
      scales[static_cast<uint64_t>(group) * scale_stride_m + row] = scale;
    }

    const float2 first_pair = make_float2(
        fminf(fmaxf(values[0] / scale, -kFp8Max), kFp8Max),
        fminf(fmaxf(values[1] / scale, -kFp8Max), kFp8Max));
    const float2 second_pair = make_float2(
        fminf(fmaxf(values[2] / scale, -kFp8Max), kFp8Max),
        fminf(fmaxf(values[3] / scale, -kFp8Max), kFp8Max));
...
extern "C" int mistralrs_fused_add_rms_norm_quantize_bf16(
    ...
    __nv_fp8_e4m3 *quantized_output, float *scales, float *inverse_rms,
    int rows, int columns, int scale_stride_m, float epsilon, void *stream) {
  if (input == nullptr || residual == nullptr || weight == nullptr ||
      ...
      columns % mistralrs::fp8::kFp8GroupSize != 0 ||
      scale_stride_m < rows || !std::isfinite(epsilon) || epsilon < 0.0f ||
  ...
  const bool stat_aligned = reinterpret_cast<uintptr_t>(input) % 16 == 0 &&
  ...
  const int stat_threads = mistralrs::fp8::rounded_block_threads(
      stat_aligned ? columns / 8 : columns);
  const auto cuda_stream = static_cast<cudaStream_t>(stream);
  if (stat_aligned) {
    mistralrs::fp8::add_rms_stat_bf16_aligned_kernel
        <<<rows, stat_threads, 0, cuda_stream>>>(
            input, residual, residual_output, inverse_rms, columns, epsilon);
  } else {
    mistralrs::fp8::add_rms_stat_bf16_kernel
        <<<rows, stat_threads, 0, cuda_stream>>>(
            input, residual, residual_output, inverse_rms, columns, epsilon);
  }
  ...
  constexpr int warps_per_block =
      mistralrs::fp8::kQuantThreads / mistralrs::fp8::kWarpSize;
  const uint64_t group_count = static_cast<uint64_t>(rows) * columns /
                               mistralrs::fp8::kFp8GroupSize;
  const int quant_blocks =
      static_cast<int>((group_count + warps_per_block - 1) / warps_per_block);
  if (quant_aligned) {
    mistralrs::fp8::rms_norm_quantize_bf16_kernel<true>
        <<<quant_blocks, mistralrs::fp8::kQuantThreads, 0, cuda_stream>>>(
            residual_output, weight, inverse_rms, quantized_output, scales,
            rows, columns, scale_stride_m);
  } else {
    mistralrs::fp8::rms_norm_quantize_bf16_kernel<false>
        <<<quant_blocks, mistralrs::fp8::kQuantThreads, 0, cuda_stream>>>(
            residual_output, weight, inverse_rms, quantized_output, scales,
            rows, columns, scale_stride_m);
  }

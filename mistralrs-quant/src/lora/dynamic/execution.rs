...
use super::{LoraRuntimeId, LoraSiteHandle};
...
impl LoraAdapterWeights {
    pub fn new(
        runtime_id: LoraRuntimeId,
        weights: Vec<(Arc<LoraSiteHandle>, LoraWeights)>,
    ) -> Result<Self> {
        let mut by_site = HashMap::with_capacity(weights.len());
        for (site, weights) in weights {

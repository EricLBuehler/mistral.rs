...
kernel void kernel_flash_attn_ext_blk(
        constant flash_attn_ext_blk_kargs & args [[buffer(0)]],
    ...
    if (tiisg == 0) s_any_unmasked = 0;
    simdgroup_barrier(mem_flags::mem_threadgroup);

    for (int jq = 0; jq < Q; ++jq) {
        const int iq = i1 * Q + jq;
        if (iq >= args.ne31) break;
        device const half * mp = (device const half *)(mask + args.nb31 * iq + args.nb32 * i2 + args.nb33 * i3) + i0 * C;
...
kernel void kernel_flash_attn_ext_bf16_dk512_dv512(
        ...
        device const char * blk              [[buffer(7)]],
...
#define NS20 FA_NS20
    ...
    threadgroup bfloat  * sk   = (threadgroup bfloat  *)(shmem_f16 + sgitg * (4 * 16 * KV) + Q * T + Q * TS);
    threadgroup bfloat  * sv   = (threadgroup bfloat  *)(shmem_f16 + sgitg * (4 * 16 * KV) + Q * T + Q * TS);

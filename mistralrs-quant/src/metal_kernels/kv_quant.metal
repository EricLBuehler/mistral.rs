...
template <typename T>
void quantize_row_q8_0_ref(const device T * x, device BlockQ8_0 * y) {
    const int qk = QK8_0;
...
template <typename T>
void dequantize_row_q8_0(device T * y, const device BlockQ8_0 * x) {
    const int qk = QK8_0;
...
instantiate_quantize_8bit(half)
instantiate_quantize_8bit(bfloat16_t)
...
template <typename T>
void quantize_row_q4_0_ref(const device T * x, device BlockQ4_0 * y) {
    const int qk = QK4_0;
    ...
    for (int j = 0; j < qk/2; ++j) {
        const float x0 = float(x[0    + j])*id;
        const float x1 = float(x[qk/2 + j])*id;
...
template <typename T>
void dequantize_row_q4_0(device T * y, const device BlockQ4_0 * x) {
    const int qk = QK4_0;
...
instantiate_quantize_4bit(half)
instantiate_quantize_4bit(bfloat16_t)
...
instantiate_dequantize_4bit(bfloat16_t)

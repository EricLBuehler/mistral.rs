...
class Idefics2VisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, patch_attention_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        posembed = self.position_embedding(position_ids)
...
class Idefics2EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        np.save("pixel_values_probe_p.npy", hidden_states.float().cpu().numpy())
        print("saved it");
        1+"A"
...
class Idefics2Model(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            import numpy as np
            print("pixel_values_start")
            np.save("pixel_values_start.npy", pixel_values.float().cpu().numpy())

...
Gemma weights are gated on Hugging Face and require one-time license acceptance per account.
...
The token is saved to `~/.cache/huggingface/token` and reused for subsequent downloads.

If you have already logged in via `huggingface-cli`, skip this step. Both tools read the same token file.

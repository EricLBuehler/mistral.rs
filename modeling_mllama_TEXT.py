...
def _prepare_cross_attention_mask(
    ...
    NEGINF_VAL=-1e15#torch.finfo(dtype).min
    cross_attention_mask = inverted_cross_attn_mask.masked_fill(
        inverted_cross_attn_mask.to(torch.bool), NEGINF_VAL
    )
    
    # apply full-row bias, which return 4D tensor of shape [B, H, S1, 1] where value is 0 if the a full row in cross attn mask's
    # last dimension contains negative infinity values, otherwise it's 1
    negative_inf_value = NEGINF_VAL
...
class MllamaTextModel(MllamaPreTrainedModel):
    ...
    def forward(
        ...
        print("saving prenorm_hs")
        np.save("/home/ubuntu/dump/truth/prenorm_hs.npy",hidden_states.cpu().float().numpy())
        print("wrote prenorm_hs")

        hidden_states = self.norm(hidden_states)

                
        print("saving norm_hs")
        np.save("/home/ubuntu/dump/truth/norm_hs.npy",hidden_states.cpu().float().numpy())
        print("wrote norm_hs")
...
class MllamaForCausalLM(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        logits: torch.Tensor = self.lm_head(hidden_states[:, -num_logits_to_keep:, :]).float()
                
        print("saving lm_hs")
        np.save("/home/ubuntu/dump/truth/lm_hs.npy",logits.cpu().float().numpy())
        print("wrote lm_hs")
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            print("saving cross_attention_states")
            np.save("/home/ubuntu/dump/truth/cross_attention_states.npy",cross_attention_states.cpu().float().numpy())
            print("wrote cross_attention_states")
        ...
        if cross_attention_mask is not None:
            ...
            if len(cache_position) > 1:
                print("saving cross_attention_mask")
                np.save("/home/ubuntu/dump/truth/cross_attention_mask.npy",cross_attention_mask.cpu().float().numpy())
                print("wrote cross_attention_mask")
                
                print("saving full_text_row_masked_out_mask")
                np.save("/home/ubuntu/dump/truth/full_text_row_masked_out_mask.npy",full_text_row_masked_out_mask.cpu().float().numpy())
                print("wrote full_text_row_masked_out_mask")
